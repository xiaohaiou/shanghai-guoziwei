...
<head>
	<meta charset="utf-8">
	<title>海航实业BIM管理数字化平台门户（体验版）</title>
	<!-- 库|插件 -->
...
<body>
	...
	<div no=text class="l_top l_top_bg2">
		...
		<p class="l_logo_p1">BIM管理数字化平台(体验版)</p>
		<p class="l_logo_p2">BIM Management Digitization Platform</p>
		...
		<div class="menu user" no="text">
			...
			<div class="avatar">
			<%-- 
					<img src="${gzwsession_users.imgPath}" onerror="to_def_img(this)" alt="头像"> --%>
			</div>
		...
		<div class="r1" no=text>
					...
					<ul class="body">
						<c:if test="${empty requestScope.missionList}">
							<c:forEach var="tl" items="${missionList}">
								<li no=text onclick="getMissionDetail(${gzwsession_users.vcEmployeeId},${tl.id});">
									<div class="content">
						...
						<div class="btn-more"><a href="/bim_task/system/index?vcEmployeeId=${gzwsession_users.vcEmployeeId}&taskOrInstruction=task">查看全部</a></div>
					</ul>
				</section>
					...
					<ul id="taskUl" class="body">
						<c:if test="${empty requestScope.taskList}">
							<c:forEach var="tl" items="${taskList}">
								<li no=text onclick="getTaskDetail(${gzwsession_users.vcEmployeeId},${tl.id});">
									<div class="content">
										<div class="title">任务名称:<c:out value="${tl.taskName}"/></div>
										<div class="info">任务内容：<span class="text"><c:out value="${tl.taskDescription}"/></span></div>
										<div class="time">任务截至时间：<c:out value="${tl.deadLine}"/></div>
						...
						<div class="btn-more"><a href="/bim_task/system/index?vcEmployeeId=${gzwsession_users.vcEmployeeId}&taskOrInstruction=instruction">查看全部 </a></div>
					</ul>
				</section>
			</div>
			<div class="c2">
			...
			<div class="c3">
				<section class="alert">
					...
					<ul no=text>
						<li>
							<div class="num" id="errComNum" onclick="openNewWin('http://10.123.32.74:8080/hhang_web/sys/bimReturn/_abnormalmessage01?vcEmployeeId=${gzwsession_users.vcEmployeeId }')">0</div>
							<div class="hr"></div>
							<div class="name" onclick="openNewWin('http://10.123.32.74:8080/hhang_web/sys/bimReturn/_abnormalmessage01?vcEmployeeId=${gzwsession_users.vcEmployeeId }')">
								工商异常
							</div>
							<i><img src="<c:url value="/css/icon/gsyc.png"/>" alt="工商异常"></i>
						</li>
						<li>
							<div class="num" id="djgNum" onclick="openNewWin('http://10.123.32.74:8080/hhang_web/sys/bimReturn/_managerCompare01?vcEmployeeId=${gzwsession_users.vcEmployeeId }')"></div>
							<div class="hr"></div>
							<div class="name" onclick="openNewWin('http://10.123.32.74:8080/hhang_web/sys/bimReturn/_managerCompare01?vcEmployeeId=${gzwsession_users.vcEmployeeId }')">董监风险</div>
							<i><img src="<c:url value="/css/icon/djfx.png"/>"  alt="董监风险"></i>
						</li>
						<li>
							<div class="num" onclick="alterNum(${gzwsession_users.vcEmployeeId},${alterNum})">${alterNum}</div>
							<div class="hr"></div>
				...
				<section class="remind tab">
					...
					<ul class="body">
						<li>
							<span onclick="openNewWin('http://10.123.32.74:8080/hhang_web/sys/bimReturn/_gsProcess01?vcEmployeeId=${gzwsession_users.vcEmployeeId }')">工商任务待处理</span>
							<i class="num over-zero" id="gsProcessNum">0</i>
						</li>
						<li>
							<span onclick="openNewWin('http://10.123.32.74:8080/hhang_web/sys/bimReturn/_gsBrand01?vcEmployeeId=${gzwsession_users.vcEmployeeId }')">商标超时提醒</span>
