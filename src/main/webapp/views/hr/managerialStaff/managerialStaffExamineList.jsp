	...
	<body>
		...
		<form id="form1"  class="row"  >
		    ...
		    <div class="form_div col-md-12">
		    	<c:if test="${fn:contains(gzwsession_code,',bimWork_glgbsSh_q,')==true }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${sys.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_glgbsSh_e,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/exam?op=report&id=${sys.id}',${sys.id },${sys.status})" >审核</a>
										</c:if>
									</c:if>	
									<c:if test="${sys.status==50115}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_glgbsSh_eback,')==true }"><!-- bimWork_glgbsSh_eback -->
