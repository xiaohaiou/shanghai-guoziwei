...
<body>
	<c:choose>
		<c:when test="${op eq 'report'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>重大事项数据上报
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>重大事项数据查看
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	</c:choose>
	<form:form id="form2" modelAttribute="entityview" >
		<div class="label_inpt_div">
			...
			<div class="model_part">
				<label class="w20">重大事项数据总数(次):</label>
				<label class="w25 setleft">${ entityview.careCount}</label>
				<label class="w20">重大事项数据参与人次(人):</label>
				<label class="w25 setleft">${ entityview.carePerson}</label>
				<label class="w20">重大事项数据人均参与度(人次):</label>
			...
			<c:if test="${ !empty entityExamineviewlist }">
				...
				<c:forEach items="${entityExamineviewlist}" var="sys" varStatus="status">
				<div class="model_part">
...
<script type="text/javascript">
		...
		$("#commit-2").click(function(){
			var url = "/shanghai-gzw/hr/employeecare/report?id=" + id;
