	...
	<body>
		...
		<form id="form1"  class="row" >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSb_query,')==true }">
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSb_new,')==true }">
			...
			<div class="tablebox">
				<table>
					<tr class="table_header">
						...
						<th style="width:33%" colspan="3">覆盖度</th>
						<th style="width:7%" rowspan="2">创建人</th>
						<th style="width:7%" rowspan="2">上报人</th>
						<th style="width:7%" rowspan="2">审核人</th>
						<th style="width:8%" rowspan="2">状态</th>
						<th style="width:14%" rowspan="2">操作</th>
					</tr>
					<tr class="table_header">
						<th style="width:7%">重大事项总数</th>
						<th style="width:7%">重大事项总参与数</th>
						<th style="width:9%">重大事项人均参与数</th>
					</tr>
					...
					<c:set var="recordNumber" value="${(requestScope.msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="resultList" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSb_view,')==true }">
									...
									<c:if test="${resultList.status==50114 || resultList.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSb_edit,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${resultList.id}',${resultList.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSb_report,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${resultList.id}',${resultList.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSb_delete,')==true }">
