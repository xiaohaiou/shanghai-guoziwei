...
<body>
			...
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>产权信息采集审核
		...
		<form:form id="form2" modelAttribute="entity" >
			...
			<div class="model_part">
				<label class="w20">劳动用工人数（人）:</label>
				<label class="w25 setleft" >${entity.workersNumber}</label> 
				<label class="w20">劳务用工人数（人）:</label>
				<label class="w25 setleft" >${entity.tworkersNumber}</label> 
			</div>
			<h3 class="data_title1">劳动生产率</h3>
			<div class="model_part">
				<label class="w20">人均收入（万元）:</label>
				<label class="w25 setleft" >${entity.averageIncome}</label> 
				<label class="w20">人均利润（万元）:</label>
				<label class="w25 setleft" >${entity.averageProfit}</label> 
				<label class="w20">营业收入（万元）:</label>
				<label class="w25 setleft" >${entity.businessIncome}</label> 
				<label class="w20">营业利润（万元）:</label>
				<label class="w25 setleft" >${entity.businessProfit}</label> 
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			...
			$.post("/shanghai-gzw/headCount/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			...
			$.post("/shanghai-gzw/headCount/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
	    		var commitresult=JSON.parse(data);
						if(commitresult.result==true)
