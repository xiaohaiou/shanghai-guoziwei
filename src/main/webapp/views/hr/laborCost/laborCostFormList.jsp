	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_rgcbzxlTb_q,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_rgcbzxlTb_n,')==true }">	
						<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_rgcbzxlTb_synchronization,')==true }">	
					...
					<c:if test="${!empty requestScope.msgPage.list}">
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td style="text-align: left;">
									${sys.monthlyBudget}万元
								</td>
								<td style="text-align: left;">
									${sys.monthlyPerform}万元
								</td>
								
								
								<td style="text-align: center;">
									${sys.capitalization}万元
								</td>
								<td style="text-align: right;">
									${sys.retainedProfits}万元
								</td>
								<td style="text-align: right;">
									${sys.laborCost}%
								</td>
		...
		*/
		...
		function del(id,status){
			win.confirm('确定要删除？',function(r){
				if(r){
					var url = "/shanghai-gzw/laborCost/delete?id=" + id+"&status="+status;
		...
		function tb(){
			...
			var url="${pageContext.request.contextPath}/laborCost/getOverviewIndustry?year="+year+"&month="+month+"&NodeId="+NodeId;
