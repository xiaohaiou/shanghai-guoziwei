...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>修改${level == 2? "二" : "三"}级风险目录
		        <i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增${level == 2? "二" : "三"}级风险目录
		        <i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
		...
		<div class="label_inpt_div">
			<div class="model_part">
				...
				<span class="w25">
					<select name="level1Id" id="sel_level1Id" class="selectpicker">
						<c:forEach items="${ctgs1}" var="result" varStatus="status">
							<option value="${result.id }" <c:if test="${result.id == entities[1].parentId}">selected="selected"</c:if>>${result.name}</option>
						</c:forEach>
					</select> 
				</span></br>
				<c:if test="${level == 2}">
				    <label class="w20"><span class="required"> * </span>二级目录:</label>
				    <input class="w25" name="name" id="inp_name" value="${ entities[1].name}" check="NotEmpty_string_._._._._."/></br>
				    <label class="w20"><span class="required"> * </span>二级目录定义:</label>
				    <textarea class="w70" rows="5" cols="" id="tex_define" name="define" style="height:5em;" maxlength="200"  check="NotEmpty_string_._._._._.">${entities[1].define}</textarea></br>
				</c:if>
				...
				<c:if test="${level == 3 }">
				    ...
				    <input class="w25" name="name" id="inp_name" value="${entitiesCount == 3 ? entities[2].name: ''}" check="NotEmpty_string_._._._._."/></br>
				    <label class="w20"><span class="required"> * </span>三级目录定义:</label>
				    <textarea class="w70" rows="5" cols="" id="tex_define" name="define" style="height:5em;" maxlength="200" check="NotEmpty_string_._._._._.">${entitiesCount == 3 ? entities[2].define: ''}</textarea></br>
				</c:if>
			</div>
			<div class="model_btn">
...
<script type="text/javascript">
	...
	function commit1(isSubmitAudit){
		...
		var level = '${level}';
		var id = '${entityId}';
		var params = {parentId: parentId, name: name, define: define, isSubmitAudit: isSubmitAudit, level: level,id:id};
	...
	function setLevel2(id){
		...
		$.get('${mapurl}/getChildren', {'parentId': id}, function(data){
			...
			$.each(data.entity, function(i, o){
			    if(o.id == selected){
			        $("#sel_level2Id").append("<option value='"+o.id+"' selected='selected'>"+o.name+"</option>");
			    }else{
