		...
		<form id="form1" class="row">
			...
			<span class="col-md-4"> 状态：
				<select name="status" class="selectpicker">
					...
					<option value="0" <c:if test="${status == 0}">selected="selected"</c:if>>未上报</option>
			...
			<div class="form_div col-md-12">
			    <c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_query,')==true}">
				     <input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_addSecondDirectory,')==true}">
				      <input type="button" value="添加二级目录" class="form_btn" id="qrybtn" onclick="mload('${mapurl}/edit?level=2')">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_addThirdDirectory,')==true}">
				      <input type="button" value="添加三级目录" class="form_btn" id="qrybtn" onclick="mload('${mapurl}/edit?level=3')">
				</c:if>
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l" varStatus="status">
						<tr>
							...
							<td style="text-align: left;">
							    <c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_show,')==true}">
								    <c:if test="${ l[7] == 1 || l[7] == 3}">
										<a href="javascript:void(0)" onclick="mload('${mapurl}/view?id=${l[0]}')">查看</a>
									</c:if> 
								</c:if>
								<c:if test="${ l[7] == 0 || l[7] == 2}">
								     <c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_show,')==true}">
								        <a href="javascript:void(0)" onclick="mload('${mapurl}/view?id=${l[0]}')">查看</a>
									 </c:if>
									 <c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_report,')==true}">
									      <a href="javascript:void(0)" onclick="mload('${mapurl}/view?id=${l[0]}&type=submit')">上报</a>
								     </c:if>
								     <c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_delete,')==true}">
									       <a href="javascript:void(0);" onclick="delCatalog('${l[0]}')">删除</a>
								     </c:if> 
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_fxmlgl_modify,')==true }">	
									          <a href="javascript:void(0)" onclick="mload('${mapurl}/edit?op=modify&id=${l[0]}')" >修改</a>
									</c:if>
								</c:if>
							</td>
						</tr>
...
<script type="text/javascript">
		...
		function delCatalog(id){
			parent.win.confirm('确定要删除风险目录吗？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/bimr/riskCatalog/delete";
					var param = {id: id};
					$.post(url, param, function(data) {
						if(data.result){
							win.successAlert('删除风险目录成功！',_query);
						}else{
