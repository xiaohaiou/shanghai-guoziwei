...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			<span class="col-md-4">
				<span class="search_title">审计实施单位: </span>
			...
			<span class="col-md-4">
				<span class="search_title">审计项目名称: </span>
				<input type="text" name="auditProjectName" value="${entity.auditProjectName}"/>
			</span>
			<span class="col-md-4">
				<span class="search_title">审计项目性质: </span>
			...
			<span class="col-md-4">
				<span class="search_title">审计起止时间: </span>
			...
			<div class="tablebox">
				<table>
					...
					<c:if test="${not empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="rx" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsjsb_query,')==true}">
										<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/question/unClosedView?id=${auditProject.id}')">查看</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
