...
<html>
	...
	<body class="hn_grey">
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">	
			        <c:if test="${fn:contains(gzwsession_code,',bimWork_msajsb_query,')==true }">
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_msajsb_new,')==true }">
						<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${pageContext.request.contextPath}/bimr/case/addCivilcase')">
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="case" varStatus="status">
							<tr>
								...
								<td>
								     <c:if test="${fn:contains(gzwsession_code,',bimWork_msajsb_show,')==true }">
									     <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/case/viewCivilcase?id=${case.id}')">查看</a>
                                    </c:if>
                                     <c:if test="${fn:contains(gzwsession_code,',bimWork_msajsb_update,')==true }">	
											<c:if test="${case.approvalState == 0 || case.approvalState == 3 }">
												<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/case/updateCivilcase?id=${case.id}')">修改</a>
											</c:if>
									</c:if>	
									 <c:if test="${fn:contains(gzwsession_code,',bimWork_msajsb_change,')==true }">		
										<c:if test="${case.approvalState == 2 }">
											<a href="javascript:;" onclick="mload('${pageContext.request.contextPath}/bimr/case/exchangeCivilcase?id=${case.id}')">变更</a>
										</c:if>
									</c:if>
									 <c:if test="${fn:contains(gzwsession_code,',bimWork_msajsb_delete,')==true }">
										<c:if test="${case.approvalState == 0 || case.approvalState == 3 }">
											<a href="javascript:;" onclick="del(${case.id})">删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
	...
	<script type="text/javascript">
		...
		{
		 var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/case/minshiSbExport";
			form.method = "post";
			form.submit();
		} */
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/bimr/case/delCivilcase?id="+id;
