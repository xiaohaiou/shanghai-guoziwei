	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<div class="form_div col-md-12">
			     <c:if test="${fn:contains(gzwsession_code,',bimWork_shhgdcsj_query,')==true}">
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l"
						...
						<tr>
							...
							<td>${l.statusName }</td>
							<td>
							    <c:if test="${fn:contains(gzwsession_code,',bimWork_shhgdcsj_show,')==true}">
								     <a href="javascript:void(0)" onclick="mload('${mapurl}/manageView?id=${l.id }')">查看</a>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_shhgdcsj_examine,')==true}">
									<c:if test="${l.status==81012 }">
										<a href="javascript:void(0)" onclick="mload('${mapurl}/manageView?id=${l.id }&type=${type }')">审核</a>
									</c:if>
								</c:if>
							</td>
						</tr>
