					...
					<c:if test="${!empty requestScope.situationPersonList }">
						<c:forEach items="${ requestScope.situationPersonList}" var="l" varStatus="status">
							<tr>
								...
								<a href="javascript:void(0)" onclick="mload('${mapurl}/correctPersonView?id=${l.id }')">修改</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
						...
						<c:forEach items="${ requestScope.suggestList}" var="l" varStatus="status">
							<tr>
								...
								<c:if test="${l.isChange == 1}">
								    <a href="javascript:void(0)" onclick="mload('${mapurl}/correctProblemView?id=${l.id }&isEdit=true')">整改</a>
								</c:if>
								</td>
							</tr>
						</c:forEach>
