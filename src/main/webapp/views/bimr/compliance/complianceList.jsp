	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<div class="form_div col-md-12">
			        <c:if test="${fn:contains(gzwsession_code,',bimWork_hgdcjbxxwh_query,')==true}">
					     <input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_hgdcjbxxwh_new,')==true}">
					     <input type="button" value="新增" class="form_btn" id="model_add" onclick="mload('${mapurl}/addOrModify?op=add')">
					</c:if>
					<input type="button" value="导出" class="form_btn" id="exportbtn" onclick="mload('${mapurl}/bimrComplianceExport')">
					<!-- mapurl = request.getContextPath() + "/bimr/compliance"; -->
					<%-- <c:if test="${fn:contains(gzwsession_code,',bimWork_hgdcjbxxwh_query,')==true}">
					     <input type="button" value="导出" class="form_btn" id="exportbtn" onclick="_export66()">
					</c:if> --%>
			</div>
		</form>
		<div class="tablebox">
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l"
						...
						<tr>
							...
							<!-- 这边是列表字段里面的查看 -->
							<td>
							    <c:if test="${fn:contains(gzwsession_code,',bimWork_hgdcjbxxwh_show,')==true}">
								     <a href="javascript:void(0)" onclick="mload('${mapurl}/view?type=${type }&id=${l.id }')">查看</a>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_hgdcjbxxwh_update,')==true}">
									<c:if test="${l.status==81010 || l.status==81011}">
										<a href="javascript:void(0)" onclick="mload('${mapurl}/addOrModify?op=modify&id=${l.id }')">修改</a>
									<!-- onclick="mload('${pageContext.request.contextPath}/bimr/case/viewCriminalcase?id=${case.id}')" -->
									</c:if>	
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_hgdcjbxxwh_delete,')==true}">
...
<script type="text/javascript">
		...
		{
		 var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/compliance/bimrComplianceExport";
			form.method = "post";
			form.submit();
		}
		...
		{
		 var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/compliance/queryExportExport";
			form.method = "post";	
			form.submit();
		}
