	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<div class="form_div col-md-12">
		    	 <c:if test="${fn:contains(gzwsession_code,',bimWork_hgdczgxxsh_query,')==true}">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l" varStatus="status">
						<tr>
							...
							<td style="text-align: left;">
							    <c:if test="${fn:contains(gzwsession_code,',bimWork_hgdczgxxsh_show,')==true}">
							          <a href="javascript:void(0)" onclick="mload('${mapurl}/correctExamineView?id=${l.id }&type=view')">查看</a>
							    </c:if>
							    <c:if test="${fn:contains(gzwsession_code,',bimWork_hgdczgxxsh_examine,')==true}">
								    <c:if test="${ l.status == 81016}">					
										<a href="javascript:void(0)" onclick="mload('${mapurl}/correctExamineView?id=${l.id }&type=examine')">审核</a>
									</c:if>
								</c:if>
							</td>
						</tr>
	...
	function saveCorrectArchive(id){
		parent.win.confirm('确定要归档合规调查整改信息吗？',function(r){
			if(r){
				...
				var url = "${pageContext.request.contextPath}/bimr/compliance/saveCorrectArchive";
