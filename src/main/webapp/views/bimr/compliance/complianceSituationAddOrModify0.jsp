...
<body style="height:500px;">
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>修改举报投诉反映问题线索核查情况
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增举报投诉反映问题线索核查情况
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	</c:choose>
	...
	<form:form id="form2" modelAttribute="entity" method="post" >
		...
		<div class="label_inpt_div">
			...
			<div class="model_part">
				<label class="w20"><span class="required"> * </span>举报投诉反映问题线索:</label>
				...
				<label class="w20"><span class="required"> * </span>问题线索是否属实:</label>
				<span class="w25">
					<select name="isTruth" id="isTruth" class="selectpicker">
						<c:forEach items="${requestScope.isTruth}" var="result">
							<option value="${result.id }"
								<c:if test="${entity.isTruth == result.id}">selected="selected"</c:if>>
				...
				<label class="w20"><span class="required"> * </span>具体事实情况:</label>
