	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
				 <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmwtwh_query,')==true}">
					...
					<c:if test="${!empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="auditProject" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmwtwh_viewQuestion,')==true}">
									     <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/question/questionList?projectId=${auditProject.id}')">问题查看</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmwtwh_viewDifficult,')==true}">
									    <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/difficult/list?projectId=${auditProject.id}')">困难查看</a>
									</c:if>
									<c:if test="${(auditProject.status == 50224 || auditProject.status == 50228) && auditProject.projectPrincipalId == vcEmployeeId}">
									    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmwtwh_fillProblem,')==true}">
										      <a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/question/newOrModify?op=add&&projectId=${auditProject.id}')">问题填写</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmwtwh_fillDifficult,')==true}">
										     <a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/difficult/newOrModify?op=add&&projectId=${auditProject.id}')">困难填写</a>
										 </c:if>
									</c:if>
								</td>
							</tr>
