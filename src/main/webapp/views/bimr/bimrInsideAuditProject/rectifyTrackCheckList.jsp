	...
	<body>
		...
		<form:form id="form1" class="row">
			...
			<div class="form_div col-md-12">
		    	<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzgfksh_query,')==true}">
					...
					<c:if test="${!empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="auditProject" varStatus="status">
							<tr>
								...
								<td>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzgfksh_showCompleteInformation,')==true}">
									   <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/rectify/feedBackCheckList?projectId=${auditProject.id}')">查看整改完成待审核信息</a>
								    </c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function _export()
		{
		 ...
		 var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/rectify/InsideExport";
			form.method = "post";
			form.submit();
		}
		...
		function _query() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/rectify/trackCheckList";
			form.method = "post";
			form.submit();
