...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
			   <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_query,')==true}">
					<input type="button" value="查询" class="form_btn" id="queryBtn"/>
			   </c:if>
			   <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_new,')==true}">
					<input type="button" value="新增" class="form_btn" id="model_add" onclick="mload('${pageContext.request.contextPath}/bimr/inside/newOrModify?op=add&type=main')"/>
			   </c:if>
			   <%-- <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_new,')==true}">
					<input type="button" value="导出" class="form_btn" id="model_add" onclick="_export()"/>
			   </c:if> --%>
			   
			   <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_export,')==true }"> 
						 <input type="button" value="审计项目台账导出" class="form_btn"  id="export1"   onclick="_export()" > 
				 </c:if>
				 <!-- 按钮 -->
				 
				 <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_export2,')==true }"> 
						 <input type="button" value="审计结果应用细化指标反馈导出" class="form_btn"  id="export1"   onclick="_export1()" > 
				 </c:if>
					...
					<c:if test="${!empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="auditProject" varStatus="status">
							<tr>
								...
								<td>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_show,')==true}">
                                    ...
                                    <c:if test="${auditProject.isChildren == 0 && auditProject.status == 50223}">
                                    	<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_update,')==true}">
											<a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/inside/newOrModify?op=modify&type=main&id=${auditProject.id}')">修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_delete,')==true}">
											<a href="javascript:void(0);" onclick="del('${auditProject.id}')">删除</a>
										</c:if>
									</c:if>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_createSubProject,')==true}">
										<c:if test="${auditProject.isChildren == 0 && vcEmployeeId == auditProject.projectPrincipalId && auditProject.status == 50224}">
											<a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/inside/newOrModify?op=add&type=child&mainId=${auditProject.id}')">子项目建立</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_createSubProject,')==true}">
										<c:if test="${auditProject.isChildren == 0 && vcEmployeeId == auditProject.projectPrincipalId && auditProject.status == 50228}">
											<a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/inside/newOrModify?op=add&type=child&mainId=${auditProject.id}')">子项目建立</a>
										</c:if>
									</c:if>								
									<c:if test="${auditProject.isChildren == 1 && auditProject.status == 50223 && vcEmployeeId == auditProject.projectPrincipalId}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_update,')==true}">
										    <a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/inside/newOrModify?op=modify&type=main&id=${auditProject.id}')">修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_delete,')==true}">
									...
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_startProject,')==true}">
										<c:if test="${auditProject.status == 50223}">
											<a href="javascript:void(0);" onclick="goin('${auditProject.id}')">项目启动</a>
										</c:if>
									</c:if>
									<c:if test="${auditProject.status == 50224 || auditProject.status == 50228}">
									    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_finishProject,')==true}">
										    <a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/inside/projectCloseApply?id=${auditProject.id}')">项目办结</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_update,')==true}">
										     <a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/inside/newOrModify?op=modify&type=main&projectisstart=y&id=${auditProject.id}')">修改</a>
									    </c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmtb_closeProject,')==true}">
										<c:if test="${auditProject.status == 50226}">
											<a href="javascript:void(0);" onclick="finished('${auditProject.id}')">项目关闭</a>
										</c:if>
									</c:if>
								</td>
							</tr>
	...
	<script type="text/javascript">
		...
		function _query() {
			...
			form.action = "${pageContext.request.contextPath}/bimr/inside/fill";
			form.method = "post";
			form.submit();
		};
		function _export() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/inside/InsideXMExport";
			form.method = "post";
			form.submit();
		};
		...
		function _export1() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/inside/auditResultsAppRefineExport";
			form.method = "post";
			form.submit();
		};
		/* /bimr/inside/InsideXMExport */
		function del(id){
		...
		function goin(id){
            parent.win.confirm('确定审计项目启动',function(r){
                if(r){
		...
		function goapply(id){
            parent.win.confirm('确定审计项目办结',function(r){
                if(r){
                    var url = "${pageContext.request.contextPath}/bimr/inside/applyClose";
                    $.post(url,{id:id}, function(data) {
                        var r = data.result;
                        if(r == 1) {
                            win.successAlert(data.message,function(){
     	...
     	function finished(id){
			parent.win.confirm('确定要关闭内部审计项目？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/bimr/inside/finished";
					$.post(url,{id:id}, function(data) {
					    var r = data.result;
						if(r == 1) {
							win.successAlert(data.message,function(){
