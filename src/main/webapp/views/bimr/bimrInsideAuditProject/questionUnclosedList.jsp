...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row">
			...
			<div class="form_div col-md-12">
			  <c:if test="${fn:contains(gzwsession_code,',bimWork_wwcsjxmfxwtcx_query,')==true}">
				<input type="button" value="查询" class="form_btn" id="btn_query" onclick="_query()">
			  </c:if>
			  
			   <c:if test="${fn:contains(gzwsession_code,',bimWork_wwcsjxmfxwtcx_query,')==true}"> 
					<input type="button" value="审计未完成整改项目统计表导出" class="form_btn" id="export1"  onclick="_export()"/>
				 </c:if>
			</div>
		</form:form>
			<div class="tablebox">
					...
					<c:if test="${not empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="q" varStatus="status">
							<tr>
								...
								<td>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_wwcsjxmfxwtcx_show,')==true}">
									     <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/question/unclosedview?id=${q[0]}')">查看</a>
								    </c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function _query() {
			...
			form.action = "${pageContext.request.contextPath}/bimr/question/unclosed";
			form.method = "post";
			form.submit();
		};
		...
		function _export() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/question/unFinishExport";
			form.method = "post";
			form.submit();
		};
