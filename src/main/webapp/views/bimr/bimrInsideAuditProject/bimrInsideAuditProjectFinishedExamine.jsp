		...
		<div class="label_inpt_div">
		  <h4><span style="font-weight: 1px;">查询审计项目</span></h4>
			<div class="model_part">
				...
				<label class="w25 setleft">
				<c:if test="${not empty entity.sjFileId}">
				...
				<label class="w25 setleft"><c:if test="${not empty entity.mailFileId}">
					<c:if test="${not empty mailFile}">
						<a href="javascript:void(0)" onclick="downloadFile('${pageContext.request.contextPath}/common/download?_url=${mailFile.filePath}&_name=${mailFile.fileName}')" target="_blank">${mailFile.fileName}</a>
					</c:if>
				</c:if></label>
				<br>
...
<script type="text/javascript">
		...
		$(".model_btn_pass").click(function () {
		     ...
		     var _url = '${pageContext.request.contextPath}/bimr/inside/finishedPass';
		 ...
		 $(".model_btn_reject").click(function () {
		     ...
		     var _url = '${pageContext.request.contextPath}/bimr/inside/finishedReject';
             var _id = $(this).attr("data-id");
             var op = $('#opinion').val();
		     $.post(_url,{id:_id,op:op},function (data) {
                 var r = data.result;
