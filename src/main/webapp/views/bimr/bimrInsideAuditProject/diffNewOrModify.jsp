...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>修改困难和措施
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span> 新增困难和措施
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	</c:choose>
	<form:form id="form2" modelAttribute="difficult" method="post">
		<form:hidden path="id" value="${difficult.id}"/>
		<form:hidden path="createPersonId" value="${difficult.createPersonId}"/>
		<form:hidden path="createPersonName" value="${difficult.createPersonName}"/>
		<form:hidden path="createDate" value="${difficult.createDate}"/>
		<form:hidden path="isDel" value="${difficult.isDel}"/>
		<div class="label_inpt_div">
			<div class="model_part">
				...
				<br>
				<label class="w20"><span class="required"> * </span>填写时间:</label>
				<input type="text" name="createDate" id="createDate" value="${difficult.createDate}" readonly="readonly" class="w25 time" check="NotEmpty"/>
				<br>
				<label class="w20"><span class="required"> * </span>困难和应对措施:</label>
				<textarea rows="3" cols="5" class="w70" name="difficultAndMeasure" id="difficultAndMeasure" placeholder="请输入困难和应对措施" maxlength="500" check="NotEmpty_string_500_._._._.">${difficult.difficultAndMeasure}</textarea><br>
			</div>
...
<script type="text/javascript">
		...
		$("#commit").click(function(){
			...
			var url = "${pageContext.request.contextPath}/bimr/difficult/saveOrUpdate";
            $.ajax({
                url : url,
                type : "POST",
                data: question,
