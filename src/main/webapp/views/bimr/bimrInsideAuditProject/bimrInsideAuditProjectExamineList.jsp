...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
			       <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsh_query,')==true}">
				...
				<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsh_export,')==true }"> 
						 <input type="button" value="审计报告台账导出" class="form_btn"  id="export1"   onclick="_export()" > 
				 </c:if>
			</div>
		</form:form>
			<div class="tablebox">
					...
					<c:if test="${not empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="auditProject" varStatus="status">
							<tr>
								...
								<td style="text-align: center;">
									${auditProject.examinePersonName}
								</td>
								<td style="text-align: center;">
								...
								<td>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsh_show,')==true}">
									      <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/inside/eview?id=${auditProject.id}')">查看</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsh_examine,')==true}">
										<c:if test="${auditProject.status==50227}">
											<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/inside/examineView?id=${auditProject.id}')">审核</a>
										</c:if>
									</c:if>
								</td>
							</tr>
	...
	<script type="text/javascript">
		...
		function _query() {
			...
			form.action = "${pageContext.request.contextPath}/bimr/inside/examine";
			form.method = "post";
			form.submit();
		};
		...
		function _export() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/inside/ExamineXMExport";
			form.method = "post";
			form.submit();
		};
		function del(id){
