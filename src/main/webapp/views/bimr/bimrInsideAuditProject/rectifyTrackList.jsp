...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row">
			...
			<div class="form_div col-md-12">
			    <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzgfk_query,')==true}"> 
					<input type="button" value="查询" class="form_btn" id="queryBtn"/>
				 </c:if>
				 <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzgfk_export2,')==true}"> 
					<input type="button" value="审计整改验收单（单一时点维度）导出" class="form_btn" id="export3"  onclick="_export()"/>
				 </c:if>
				 
				 <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzgfk_export3,')==true}"> 
					<input type="button" value="审计整改验收单（单一项目维度）导出" class="form_btn" id="export4"  onclick="_export1()"/>
				 </c:if>
				 
				 <%-- <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzbwh_export,')==true }"> 
						 <input type="button" value="总监工作导出" class="form_btn"  id="export1"   onclick="_export()" > 
				 </c:if> --%>
			</div>
		</form:form>
			<div class="tablebox">
				<table>
					<tr class="table_header">
						...
						<th>审计项目编号</th>
						<th style="width:6%;">被审计单位</th>
						<th style="width:6%;">审计项目名称</th>
						<th>审计项目性质</th>
						<th>审计起止时间</th>
						<th>子项目</th>
						<th>项目负责人</th>
					...
					<c:if test="${!empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="auditProject" varStatus="status">
							<tr>
								...
								<td>
								     <c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmzgfk_showChangeInformation,')==true}"> 
									      <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/rectify/feedBackList?projectId=${auditProject.id}')">查看整改信息</a>
								     </c:if> 
	...
	<script type="text/javascript">
		...
		function _query() {
			...
			form.action = "${pageContext.request.contextPath}/bimr/rectify/trackList";
			form.method = "post";
			form.submit();
		};
		...
		function _export() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/rectify/TimeDimension";
			form.method = "post";
			form.submit();
		};
		function _export1() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/rectify/ProjectDimension";
			form.method = "post";
			form.submit();
		};
		function del(id){
