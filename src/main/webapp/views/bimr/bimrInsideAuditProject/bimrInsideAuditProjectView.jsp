...
<body>
	<h4>
		<c:if test="${type eq 'main'}">
			<span class="glyphicon glyphicon-pencil"></span>审计项目查看
			<i class="iconfont icon-guanbi"></i>
		</c:if>	
		<c:if test="${type eq 'child'}">
			<span class="glyphicon glyphicon-pencil"></span>审计子项目查看
			<i class="iconfont icon-guanbi"></i>
		</c:if>	
	</h4>
	<form:form id="form2" modelAttribute="entity" >
		<div class="label_inpt_div">
		  <h4><span style="font-weight: 1px;">审计项目填报</span></h4>
			...
			<h4><span style="font-weight: 1px;">审计项目困难与措施</span></h4>
			<div class="tablebox">
				<table style="width:100%;">
					<tr class="table_header">
						<th style="width: 8%">序号</th>
						<th style="width: 20%;">项目名称</th>
						<th style="width: 38%;">困难与应对措施</th>
					</tr>
					<c:if test="${not empty difficult_list}">
						<c:forEach items="${difficult_list}" var="d" varStatus="status">
							<tr>
								<td>${status.index + 1}</td>
								<td>${d.projectName}</td>
								<td>${d.difficultAndMeasure}</td>
							</tr>
						</c:forEach>
					</c:if>
				</table>
