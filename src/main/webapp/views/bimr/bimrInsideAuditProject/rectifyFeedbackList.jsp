						...
						<c:forEach items="${ requestScope.msgPage.list}" var="r" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${r.status == 2}">
										<c:if test="${requestScope.Huser.vcEmployeeId == r.rectifyTracerId}">
											<a href="javascript:void(0);" onclick="mload('${pageContext.request.contextPath}/bimr/rectify/feedBackModify?rectifyId=${r.id}')">整改反馈</a>
											<a href="javascript:void(0);" onclick="applyFinish('${r.id}')">整改完成申请</a>
										</c:if>
									</c:if>
								</td>
							</tr>
