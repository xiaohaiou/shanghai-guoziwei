...
<body>
	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<span class="col-md-4"><span class="search_title">资源状态：</span>
			...
			<span class="col-md-4"><span class="search_title">资源编号：</span> <input type="text" name="eventNum" value="${entity.eventNum}"  class="w25"/> </span> 
			<span class="col-md-4"><span class="search_title">资源名称：</span> <input type="text" name="eventTitle" value="${entity.eventTitle}"  class="w25"/> </span> 
			<span class="col-md-4"><span class="search_title">资源类型：</span>
			...
			<div class="form_div col-md-12">
					...
					<input type="button" value="新增" class="form_btn" id="qrybtn" onclick="">				
			</div>
		</form>
		<div class="tablebox">
			<table>
				<tr class="table_header">
					...
					<th style="width:10%" >资源上报时间</th>
					<th style="width:15%" >资源上报单位名称</th>
					<th style="width:10%" >资源名称</th>
					<th style="width:10%" >资源类型</th>
					<th style="width:10%" >资源编号</th>
					<th style="width:8%" >资源状态</th>
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l" varStatus="status">
						<tr>
							...
							<%-- <td style="text-align: center;">${l.completeTime}</td> --%>
							...
							<td style="text-align: left;">
								         <a href="javascript:void(0)" onclick="mload('${mapurl}/view?id=${l.id }&op=feedbackreport')">查看</a>
								    <c:if test="${l.status == 82102}">
										<a href="javascript:void(0)" onclick="mload('${mapurl}/feedback?id=${l.id }&op=feedbackreport')">反馈上报</a>
									</c:if>
									<c:if test="${op == 'examine' && l.status == 82108}">
										<a href="javascript:void(0)" onclick="mload('${mapurl}/feedback?id=${l.id }&op=feedbackexamine')">反馈审核</a>
									</c:if>
								<%--
