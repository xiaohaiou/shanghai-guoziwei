	...
	<div class="table_content">
		...
		<div class="tablebox">
			<table>
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l" varStatus="status">
						<tr>
							...
							<td style="text-align: left;">${l.eventTitle}</td>
							<td style="text-align: left;">${l.eventNum}</td>
							...
							<td style="text-align: left;">
							    <a href="javascript:void(0)" onclick="mload('${mapurl}/view?id=${l.eventid}')">查看</a>
								<c:if test="${l.status == 82104}">
									<a href="javascript:void(0)" onclick="mload('${mapurl}/riskEventExamine?op=riskexamine&id=${l.eventid}&trackid=${l.id}')">风险审核</a>
								</c:if>	
								<c:if test="${l.status == 82104}">
									<a href="javascript:void(0)" onclick="mload('${mapurl}/riskEventExamine?op=riskback&id=${l.eventid}&trackid=${l.id}')">风险退回</a>
								</c:if>	
							</td>
						</tr>
					</c:forEach>
				...
				<c:if test="${empty requestScope.msgPage.list}">
					<tr>
						<td colspan="9" align="center">查询无记录</td>
					</tr>
				</c:if>
			</table>
