...
<body>
	<c:if test="${op eq 'relevancelist'}">
	<h4>数据看报</h4>
	</c:if>
	<c:if test="${op != 'relevancelist'}">
	<h4>数据看报查询</h4>
	</c:if>
	<div class="table_content">
		<form id="form1" class="row">
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_fxsjglml_query,')==true}">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l" varStatus="status">
						<tr>
							...
							<td style="text-align: center;">
								<c:if test="${fn:contains(gzwsession_code,',bimWork_fxsjglml_show,')==true}">
							        <a href="javascript:void(0)" onclick="mload('${mapurl}/relevanceaddOrModify?op=view&id=${l.id }')">查看</a>
							    </c:if>
							    <c:if test="${fn:contains(gzwsession_code,',bimWork_fxsjglml_relete,')==true}">
									    <c:if test="${op eq 'relevancelist'}">
										    <c:if test="${ l.relevancestatus ==1 || l.relevancestatus == 2  || l.relevancestatus ==5}">
												<a href="javascript:void(0)" onclick="mload('${mapurl}/relevanceaddOrModify?op=relevance&id=${l.id }')">关联</a>
											</c:if>
										</c:if>
								</c:if>
							</td>
