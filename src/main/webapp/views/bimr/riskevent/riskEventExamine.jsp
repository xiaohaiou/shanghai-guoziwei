...
<c:choose>
		<c:when test="${op eq 'event'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>事件审核
		...
		<c:when test="${op eq 'risk'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>风险审核
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
	...
	<form:hidden path="id"/>
		<div class="label_inpt_div">
			...
			<div class="model_part">
				...
				<input class="w25" name="reportTime" id="reportTime" value="${entity.reportTime}" readonly="true"/>
		...
		<%--		 <c:if test="${entity.ishighunit == 1}">
				...
				<input type="text" name="insidetrackpersonname" id="" value="${entity.insidetrackpersonname}" class="w25" check="NotEmpty" readonly="readonly"/>
							...
							<c:forEach items="${ requestScope.adoptStrategyList}" var="l"
								...
								<tr>
									<td style="text-align: left;">
										<select name="adoptStrategy" class="selectpicker" id="adoptStrategy">
											<option value="1" <c:if test="${l.adoptStrategy == 1}">selected="selected"</c:if>>专业公司风控部负责</option>
											<option value="2" <c:if test="${l.adoptStrategy == 2}">selected="selected"</c:if>>核心企业风控部负责</option>
											<option value="3" <c:if test="${l.adoptStrategy == 3}">selected="selected"</c:if>>产业集团风控部负责</option>
										</select> 
									</td>
			...
			<c:if test="${op eq 'eventexamine' || op eq 'eventback'}">
				<h3 class="data_title1">采取策略&nbsp;&nbsp;</h3>
				<div class="tablebox">
					<table id="personTab">
						<tr class="table_header">
							<th >采取策略</th>
						...
						<c:if test="${!empty requestScope.adoptStrategyList }">
							<c:forEach items="${ requestScope.adoptStrategyList}" var="l"
								varStatus="status">
								<tr>
									<td style="text-align: left;"><input name="adoptStrategy" value="${l.adoptStrategy }" class="w25" style="width:100% !important; " readonly="readonly"></td>
									<td style="text-align: left;"><input name="responsibleCompName" value="${l.responsibleCompName }" class="w25" style="width:100% !important; " readonly="readonly"></td>
								</tr>
							</c:forEach>
						</c:if>
					</table>
			...
			<c:if test="${op eq 'riskexamine' || op eq 'riskback'}">
				<h3 class="data_title1">采取策略&nbsp;&nbsp;</h3>
				<div class="tablebox">
					<table id="personTab">
						<tr class="table_header">
						...
						<tr>
							<td style="text-align: left;"><input name="takestrategytxt" value="${track.takestrategytxt }" class="w25" style="width:100% !important; " readonly="readonly"></td>
							<td style="text-align: left;"><input name="takestrategyremark" value="${track.takestrategyremark }" class="w25" style="width:100% !important; " readonly="readonly"></td>
							<input type="hidden" value="${track.id}" name="trackid" id="trackid">
						</tr>
					</table>
				</div>
			</c:if>
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			if(check())
			{
				...
				var url = "${pageContext.request.contextPath}/bimr/riskEvent/riskeventcommitexam?id="+id+"&examStr="+examStr+"&type="+type+"&examResult="+examResult+"";
				$.ajax({  
				     url : url,  
				     type : "POST",  
				     data: formData,
		...
		$("#commit-3").click(function(){
			if(check())
			{
				$.post("${pageContext.request.contextPath}/bimr/riskEvent/riskeventcommitexam",{id:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116,type:"riskexamine",trackid:$("#trackid").val()},function(data){
		...
		$("#commit-4").click(function(){
			if(check())
			{
				$.post("${pageContext.request.contextPath}/bimr/riskEvent/riskeventcommitexam",{id:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117,type:"riskback",trackid:$("#trackid").val()},function(data){
		    		var commitresult=JSON.parse(data);
							if(commitresult.result==true)
								win.successAlert('退回成功！',function(){
										parent.location.reload(true);
		...
		function addPerson(){
			...
			var newPerson="<tr>"
							+"<td style=\"text-align: left;\">"
								+"<select name=\"adoptStrategy\" class=\"selectpicker\" id=\"adoptStrategy\">"
											+"<option value=\"1\">专业公司风控部负责</option>"
											+"<option value=\"2\">核心企业风控部负责</option>"
											+"<option value=\"3\">产业集团风控部负责</option>"
										+"</select> "
							+"</td>"
