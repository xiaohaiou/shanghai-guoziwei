	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
			        <c:if test="${fn:contains(gzwsession_code,',bimWork_ygwz_query,')==true}">
					    <input type="button" value="查询" class="form_btn" id="queryBtn">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_ygwz_new,')==true}">
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="duty" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_ygwz_show,')==true}">
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=check&id=${duty.id}',${duty.id},'check')">查看</a>
									</c:if>
									<c:if test="${duty.status==50114 || duty.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_ygwz_update,')==true}">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/addOrModify?op=modify&id=${duty.id}',${duty.id},'modify')" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_ygwz_report,')==true}">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=reported&id=${duty.id}',${duty.id},'reported')">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_ygwz_delete,')==true}">
											<a href="javascript:;" onclick="del(${duty.id})">删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
