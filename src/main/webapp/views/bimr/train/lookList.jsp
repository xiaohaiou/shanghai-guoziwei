...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
		     	<c:if test="${fn:contains(gzwsession_code,',bimWork_hgpxck_query,')==true}">
					<input type="button" value="查询" class="form_btn" id="queryBtn">
				</c:if>
			    <c:if test="${fn:contains(gzwsession_code,',bimWork_hgpxck_export,')==true}">
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="train" varStatus="status">
							<tr>
								...
								<td>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_hgpxck_show,')==true}">
	...
	<script type="text/javascript">
		...
		function _export() {
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/bimr/train/export";
			form.method = "post";
			form.submit();
		};
		function del(id){
