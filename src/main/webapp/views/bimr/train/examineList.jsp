	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
			    <c:if test="${fn:contains(gzwsession_code,',bimWork_hgpxsh_query,')==true}">
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="train" varStatus="status">
							<tr>
								...
								<td>
								     <c:if test="${fn:contains(gzwsession_code,',bimWork_hgpxsh_show,')==true}">
									      <a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=check&id=${train.id}',${train.id},'check')">查看</a>
									 </c:if>
									 <c:if test="${fn:contains(gzwsession_code,',bimWork_hgpxsh_examine,')==true}">
										<c:if test="${train.status==50113 }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/examine?id=${train.id}',${train.id},'examine')">审核</a>
										</c:if>
									  </c:if>
								</td>
							</tr>
