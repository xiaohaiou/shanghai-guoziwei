	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_htfbwh_query,')==true }"> 
					<input type="button" value="查询" class="form_btn" id="queryBtn">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_htfbwh_new,')==true }"> 
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="contractModel" varStatus="status">
							<tr>
								...
								<td>
								    <c:if test="${fn:contains(gzwsession_code,',bimWork_htfbwh_show,')==true }">
									    <a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=check&id=${contractModel.id}',${contractModel.id},'check')">查看</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_htfbwh_update,')==true }">
									      <a href="javascript:void(0)" onclick="checkdata('${mapurl}/addOrModify?op=modify&id=${contractModel.id}',${contractModel.id},'modify')" >修改</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_htfbwh_delete,')==true }">
									     <a href="javascript:;" onclick="del(${contractModel.id})">删除</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
