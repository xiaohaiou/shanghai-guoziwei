			...
			<div class="form_div col-md-12">
					<input type="button" value="查询" class="form_btn" id="qrybtn"
					 ...
					 <input type="button" value="新增"
						class="form_btn" id="addbtn"
						onclick="mload('/shanghai-gzw/bimr/contractmonth/addOrModify?op=add')">
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="resultList"
						...
						<tr>
							...
							<!-- 操作按钮-->
							<td>
							        <a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/bimr/contractmonth/view?id=${resultList.id}')">查看</a>
								<!-- 如果--> 
								<c:if test="${ resultList.status==50112 || resultList.status==50114}">
											<a href="javascript:void(0)"
												onclick="mload('${pageContext.request.contextPath}/bimr/contractmonth/reportview?type=${type }&id=${resultList.id}')">上报</a>
										<a href="javascript:void(0)"
											onclick="deleteData('${pageContext.request.contextPath}/bimr/contractmonth/del?id=${resultList.id}&lastModifyDate=${resultList.lastModifyDate }')">删除</a>
								</c:if></td>
						</tr>
					</c:forEach>
				</c:if>
