...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>编辑志愿服务与员工关爱数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增志愿服务与员工关爱数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	</c:choose>
	<form:form id="form2" modelAttribute="entityview" >
		...
		<div class="label_inpt_div">
			...
			<div class="model_part">
				<label class="w20"><span class="required"> * </span>志愿服务活动累计次数(次):</label>
				<form:input class="w25" id="serviceCount" path="serviceCount" check="NotEmpty_int_10_0_0+_._." />
				<label class="w20"><span class="required"> * </span>志愿服务活动累计时长(小时):</label>
				<form:input class="w25" id="serviceDuration" path="serviceDuration" check="NotEmpty_double_10_4_0+_._." />
				<label class="w20"><span class="required"> * </span>志愿服务活动累计人次(人):</label>
				<form:input class="w25" path="servicePerson" check="NotEmpty_int_10_0_0+_._."/>
			</div>
			<h3 class="data_title1">员工关爱活动覆盖度</h3>
			<div class="model_part">
				<label class="w20"><span class="required"> * </span>员工关爱活动总数(次):</label>
				<form:input class="w25" id="careCount" path="careCount" check="NotEmpty_int_10_0_0+_._."/>
				<label class="w20"><span class="required"> * </span>员工关爱活动总参与人次(人):</label>
				<form:input class="w25" id="carePerson" path="carePerson" check="NotEmpty_int_10_0_0+_._."/>
				<label class="w20"><span class="required"> * </span>员工关爱活动人均参与度(人次):</label>
				<form:input class="w25" id="careParticipation" path="careParticipation" readonly="true"/>
			</div>
...
<script type="text/javascript">
		...
		function calrate(){
			...
			if(careCount != "" && carePerson != "" && carePerson != '0'){
				var rate=(parseFloat(carePerson)/parseFloat(careCount)).toFixed(4);
		...
		$("#commit-1").click(function(){
			...
			var url = "${pageContext.request.contextPath}/social/voluntary/save";
		...
		$("#commit-2").click(function(){
			...
			var url = "${pageContext.request.contextPath}/social/voluntary/saveandreport";
			var formData = new FormData($("#form2")[0]);
			$.ajax({  
				url : url,  
				type : "POST",  
