	...
	<body>
		...
		<form id="form1" class="row" >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_consensusSb_query,')==true }">
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_consensusSb_new,')==true }">
					...
					<c:set var="recordNumber" value="${(requestScope.msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="resultList" varStatus="status">
							<tr>
								...
								<td style="text-align: center;">
									第${resultList.week }周(${resultList.dateFrom}~${resultList.dateTo})
								</td>
								<td style="text-align: right;">
									${resultList.consensusCount }
								</td>
								<td style="text-align: right;">
									${resultList.reportConut }
								</td>
								<td style="text-align: center;">
