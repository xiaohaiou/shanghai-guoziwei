	...
	<body>
		...
		<form id="form1"  class="row" >
			...
			<div class="form_div col-md-12">
				...
				<c:if test="${fn:contains(gzwsession_code,',bimWork_brandSb_new,')==true }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="resultList" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${resultList.status==50114 || resultList.status==50112 || resultList.status==50115 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_brandSb_edit,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${resultList.id}',${resultList.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_brandSb_report,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${resultList.id}',${resultList.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_brandSb_delete,')==true }">
