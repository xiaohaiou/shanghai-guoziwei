	...
	<body>
		...
		<form id="form1"  class="row" >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSb_query,')==true }">
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<!-- <c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSb_new,')==true }">
			...
			<div class="tablebox">
				<table>
					<tr class="table_header">
						...
						<th style="width:21%" colspan="3">党建工作信息采集</th>
						...
						<th style="width:22%" rowspan="2">操作</th>
					</tr>
					<tr class="table_header">
						<th style="width:7%">党建工作信息采集1</th>
						<th style="width:7%">党建工作信息采集2</th>
						<th style="width:7%">党建工作信息采集3</th>
					...
					<c:set var="recordNumber" value="${(requestScope.msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="resultList" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSb_view,')==true }">
									...
									<c:if test="${resultList.status==50114 || resultList.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSb_edit,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${resultList.id}',${resultList.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSb_report,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${resultList.id}',${resultList.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSb_delete,')==true }">
