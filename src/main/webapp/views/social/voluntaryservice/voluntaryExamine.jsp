...
<body>
	<h4>
		<span class="glyphicon glyphicon-pencil"></span>党建信息数据审核
	...
	<form:form id="form2" modelAttribute="entityview" >
		...
		<div class="label_inpt_div">
			...
			<h3 class="data_title1">党建信息活动量</h3>
			<div class="model_part">
				<label class="w20">党建信息累计次数(次):</label>
				<label class="w25 setleft">${ entityview.serviceCount}</label>
				<label class="w20">v累计时长(小时):</label>
				<label class="w25 setleft">${ entityview.serviceDuration}</label>
				<label class="w20">党建信息累计人次(人):</label>
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			$.post("/shanghai-gzw/social/voluntaryservice/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			$.post("/shanghai-gzw/social/voluntaryservice/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
				var commitresult=JSON.parse(data);
				if(commitresult.result==true){
					win.successAlert('保存成功！',function(){
									parent.location.reload(true);
