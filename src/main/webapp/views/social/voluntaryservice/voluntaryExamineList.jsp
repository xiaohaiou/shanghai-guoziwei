		...
		<div class="table_content">
			<form id="form1" class="row"  >
				...
				<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSh_query,')==true }">
			...
			<div class="tablebox">
				<table>
					<tr class="table_header">
						...
						<th style="width:21%" colspan="3">党建信息采集</th>
					...
					<c:set var="recordNumber" value="${(requestScope.msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="resultList" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSh_view,')==true }">
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=modify&id=${resultList.id}',${resultList.id })" >查看</a>
									</c:if>
								<!-- 
									<c:if test="${resultList.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSh_exam,')==true }">
									...
									<c:if test="${resultList.status==50115}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_voluntaryNewSh_examback,')==true }"><!-- bimWork_voluntaryNewSh_examback -->
