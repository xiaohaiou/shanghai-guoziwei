					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="auditProject" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsjsb_query,')==true}">
										<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/queryPage/riskcontrol/auditProject/view?op=check&id=${auditProject.id}')">查看</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/riskcontrol/auditProject/delete?id="+id;
