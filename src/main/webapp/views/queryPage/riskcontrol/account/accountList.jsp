...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			<span class="col-md-4">
				<span class="search_title">投诉名称：</span>
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="account" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_hgtzsjsb_query,')==true}">
										<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/queryPage/riskcontrol/account/view?op=check&id=${account.id}')">查看</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		$('.clear').on('click',function(){
			$("#complainedPersonDeptName").val("");
			$("#complainedPersonDept").val("");
		});
	</script>
</html>