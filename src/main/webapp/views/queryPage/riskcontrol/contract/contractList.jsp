					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="contract" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_htsjsb_query,')==true}">
										<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/queryPage/riskcontrol/contract/view?op=check&id=${contract.id}')">查看</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/riskcontrol/contract/delete?id="+id;
