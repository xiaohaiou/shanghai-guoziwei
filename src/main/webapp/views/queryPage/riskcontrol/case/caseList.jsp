...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
				<span class="col-md-4">
					<span class="search_title">案件所属：</span>
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="cases" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_ajxxsjsb_query,')==true}">
										<a href="javascript:void(0)" onclick="mload('${pageContext.request.contextPath}/queryPage/riskcontrol/case/view?op=check&id=${cases.id}')">查看</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/riskcontrol/case/delete?id="+id;
