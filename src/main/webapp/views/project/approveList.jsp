	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zdgcsjsh_query,')}">
					...
					<c:if test="${!empty requestScope.msgPage.list }">
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_zdgcsjsh_view,')}">
										<a  href="javascript:;" id="qrybtn" onclick="viewValidate('${sys.id}')">查看</a>
									</c:if>
									<c:if test="${ sys.reportStatus !=3}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_zdgcsjsh_exam,')}">
												<a  href="javascript:;" id="qrybtn" onclick="mload('${pageContext.request.contextPath}/project/approve/_approveDetail?id=${sys.id}')">审核</a>
										</c:if>
									</c:if>
	...
	<script type="text/javascript">
		...
		function approveValidate(id){
			...
			$.post(url, function(data) {
				...
				}else if(project.reportStatus == 0 || project.reportStatus == 3){
					win.errorAlert("该数据已被操作",function(){
						_query();
					});
				}else{
					mload('${pageContext.request.contextPath}/project/approve/_approveDetail?id='+id);
				}
			});
		}
	</script>
