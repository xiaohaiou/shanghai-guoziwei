...
<body>
			...
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>薪酬数据采集审核
			</h4>
		...
		<form:form id="form2" modelAttribute="entity" >
			...
			<h3 class="data_title1">酷铺商贸市场占有率</h3>
			<div class="model_part">
				<label class="w20">拓展商家数（家）:</label>
				<label class="w25 setleft" >${entity.shopNumber}</label> 
				<label class="w20">全国同类型门店数（家）:</label>
				<label class="w25 setleft" >${entity.shopCardinalNumber}</label> 
				<label class="w20">市场占有率（%）:</label>
				<label class="w25 setleft" >${entity.marketShare}</label> 
				<label class="w20">酷铺商贸市场占有率年度目标（%）:</label>
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			...
			$.post("/shanghai-gzw/sixRateKp/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			...
			$.post("/shanghai-gzw/sixRateKp/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
	    		var commitresult=JSON.parse(data);
						if(commitresult.result==true)
