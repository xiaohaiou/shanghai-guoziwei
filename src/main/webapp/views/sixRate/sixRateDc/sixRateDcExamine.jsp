...
<body>
		...
		<h4>
			<span class="glyphicon glyphicon-pencil"></span>信访数据采集
			...
			<form:form id="form2" modelAttribute="entity" >
				...
				<h3 class="data_title1">信访数据采集1</h3>
				<div class="model_part">
					<label class="w20">信访数据采集2:</label>
					<label class="w25 setleft" >${entity.marketingAmountHh}</label> 
					<label class="w20">信访数据采集3:</label>
					<label class="w25 setleft" >${entity.marketingAmountNationwide}</label> 
					<label class="w20">信访数据采集4:</label>
					<label class="w25 setleft" >${entity.marketShareDc}</label> 
					<label class="w20">海信访数据采集5:</label>
					<label class="w25 setleft" >${entity.marketShareYears}</label> 
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			...
			$.post("/shanghai-gzw/sixRateDc/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			...
			$.post("/shanghai-gzw/sixRateDc/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
	    		var commitresult=JSON.parse(data);
						if(commitresult.result==true)
