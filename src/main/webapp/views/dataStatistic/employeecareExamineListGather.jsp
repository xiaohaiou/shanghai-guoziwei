		...
		<div class="table_content">
			<form id="form1" class="row"  >
				...
				<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSh_query,')==true }">
			...
			<div class="tablebox">
				<table>
					<tr class="table_header">
						...
						<th style="width:23%" colspan="3">重大事项数据采集审核1</th>
					...
					<c:set var="recordNumber" value="${(requestScope.msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="resultList" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSh_view,')==true }">
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=modify&id=${resultList.id}',${resultList.id })" >查看</a>
									</c:if>
									<%-- <c:if test="${resultList.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSh_exam,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/exam?id=${resultList.id}',${resultList.id })" >审核</a>
										</c:if>
									</c:if>
									<c:if test="${resultList.status==50115 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_employeeCareSh_examback,')==true }"><!-- bimWork_employeeCareSh_examback -->
