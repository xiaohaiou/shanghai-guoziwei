							...
							<tr>
								...
								<td>
									...
									<c:if test="${auditProject.status==50114 || auditProject.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsjsb_edit,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/addOrModify?op=modify&id=${auditProject.id}',${auditProject.id},'modify')" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsjsb_report,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=reported&id=${auditProject.id}',${auditProject.id},'reported')">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_sjxmsjsb_delete,')==true }">
											<a href="javascript:;" onclick="del(${auditProject.id})">删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
