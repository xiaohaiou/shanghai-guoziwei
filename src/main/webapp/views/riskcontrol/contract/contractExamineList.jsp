	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_htsjsh_query,')==true}">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="contract" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${contract.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_htsjsh_exam,')==true}">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/examine?id=${contract.id}',${contract.id},'examine')">审核</a>
										</c:if>
									</c:if>
								</td>
							</tr>
