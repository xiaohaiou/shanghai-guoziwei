...
<body>
	<c:choose>
		<c:when test="${op eq 'check'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>查看合规台账
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>上报合规台账
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	</c:choose>
	<form:form id="form2" modelAttribute="riskControlAccount" method="post">
		<div class="label_inpt_div">
			<div class="model_part">
				...
				<label class="w25 setleft"><c:if test="${riskControlAccount.isFollowupInvestigation eq 1}">是</c:if><c:if test="${riskControlAccount.isFollowupInvestigation eq 0}">否</c:if><c:if test="${riskControlAccount.isFollowupInvestigation eq 2}">待定</c:if></label>
				...
				<div id="jbdcnr" style="display:inline;text-align:right;">
					<label class="w20" style="text-align: right;">举报调查具体内容:</label>
					<label class="w70 setleft">${ riskControlAccount.reportInvestigation}</label>
				</div>
			</div>
			<div id="hgdcapydc">
...
<script type="text/javascript">	
		$("#commit-3").click(function(){
			exam("${riskControlAccount.id}");
			return false;
		});
		function exam(id){
			$.blockUI({ message: '提交中', css: { width: '275px' } });
			var url = "/shanghai-gzw/riskcontrol/account/reported?id=" + id;
