						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${sys.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_zrsSh_e,')==true }">	
											
										</c:if>
									</c:if>		
									<c:if test="${sys.status==50115}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_zrsSh_eback,')==true }">	<!-- bimWork_zrsSh_eback -->
