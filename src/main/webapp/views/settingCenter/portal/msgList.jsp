...
<html>
	...
	<body>
		<div class="top-title d-position ">
			<h1 style="padding-top:30px;">公告<a class="bg-lightgreen"  href="javascript:;" onclick="mload('${pageContext.request.contextPath}/portal/msg/_add1')">新增</a></h1>
		</div>
		...
		<div class="middle-content clearfix">
			...
			<div class="white-bg">
				...
				<div class="seachbox ">
					...
					<div class="col-sm-4">
						<form id="form1" action="${pageContext.request.contextPath}/portal/msg/_query" modelAttribute="portalMsg" >
							<input type="submit" id="query" name="query" style="display:none"/>
							<div class="input-box">
								<input placeholder="公告主题" type="text" id="title" name="title" value="${portalMsg.title }" class="form-control">
							...
							<c:if test="${!empty requestScope.msgList }">
								...
								<c:forEach items="${ requestScope.msgList}" var="msg" varStatus="status">
									<tr>
										...
										<td>
											<c:if test="${msg.isIssue == 1}">
												是
											</c:if>
											<c:if test="${msg.isIssue == 0}">
												否
											</c:if>
										</td>
										<td>
											<c:if test="${msg.isIssue == 0}">
												<a href="javascript:;" onclick="issue('${msg.id}')">发布</a>
											</c:if>
											<c:if test="${msg.isIssue == 1}">
												<a href="javascript:;" onclick="issueNo('${msg.id}')">撤回</a>
											</c:if>
											<a href="javascript:;" onclick="mload('${pageContext.request.contextPath}/portal/msg/_edit?id=${msg.id }')">编辑</a>
											<a href="javascript:;" onclick="del('${msg.id}')">删除</a>
										</td>
									</tr>
								</c:forEach>
							</c:if>
	...
	<script type="text/javascript">
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/portal/msg/_del?id=" + id;
		...
		function issue(id){
			parent.win.confirm('确定要发布？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/portal/msg/_issue?id=" + id;
					$.post(url, function(data) {
		...
		function issueNo(id){
			parent.win.confirm('确定要撤回？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/portal/msg/_issueNo?id=" + id;
					$.post(url, function(data) {
		...
		function prev(){
			...
			if(pageNum == 0){
	     	...
	     	var url = "${pageContext.request.contextPath}/portal/msg/_query?pageNum="+pageNum+"&"+entity;
