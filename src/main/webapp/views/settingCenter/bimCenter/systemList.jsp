...
<html>
	...
	<body>
		...
		<div class="table_content">
			<form id="form1" action="${pageContext.request.contextPath}/bimCenterSystem/_list" class="row">
				<span class="col-md-3">系统全称：<input type="text" name="sysFulName" value="${system.sysFulName}"></span>
				<span class="col-md-3">系统简称：<input type="text" name="sysShortName" value="${system.sysShortName}"></span>
				...
				<div class="form_div col-md-2">
					...
					<input type="button" value="新增" class="form_btn" id="model_add" onclick="mload('${pageContext.request.contextPath}/bimCenterSystem/_edit')">
				</div>
			</form>
			<div class="tablebox">
				<table id="table1">
					...
					<c:if test="${!empty requestScope.systemList }">
						<c:forEach items="${ requestScope.systemList}" var="t" varStatus="status">
							<tr>
								<td>
								...
								<td>
									...
									<a href="javascript:;" onclick="mload('${pageContext.request.contextPath}/bimCenterSystem/_edit?id=${t.id }')">编辑</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
	...
	<script type="text/javascript">
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/bimCenterSystem/_del?id=" + id;
