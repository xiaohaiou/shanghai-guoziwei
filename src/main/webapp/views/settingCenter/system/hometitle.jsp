...
<body class="hn_page" >
	...
	<div no=text class="l_top">
		...
		<img src="${pageContext.request.contextPath}/img/hn_wl_logo.png"
			alt="海航物流运营管控平台" class="l_logo" />
		<p class="l_logo_p1">运营管控平台 V1.0</p>
		<p class="l_logo_p2">OPERATION CONTROL PLATFORM</p>

		<div class="menu time"></div>
		<div class="menu user" no="text">
			<div>
				<div class="name">${sessionScope.portal_users_session.vcName}</div>
				<div class="title">海航物流运行总裁</div>
			</div>
			<div class="avatar">
				<img
					src="<c:url value="http://file.hna.net/image/headimage/emp/${fn:substring(sessionScope.portal_users_session.vcEmployeeId,0,6)}0000/${sessionScope.portal_users_session.vcEmployeeId}.jpg"/>"
					onerror="to_def_img(this)" alt="头像">
			</div>
		</div>
	
		<!-- 时间 -->
		<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_show')}">
			<a class="menu i i_btn" no=text href="${pageContext.request.contextPath}/ssoout/outsystem?url=bim_show/index/index?pageId=1" title="BIM运营管控平台">
				<i class="iconfont icon-shouye"></i>
			</a> 
		</c:if>
		<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_task')}">	
			<a class="menu i i_btn" no=text onclick="openzhilinwindow();" href="javascript:void(0);" title="指令中心">
				<i class="iconfont icon-zhiling"></i><div id="zlmnum" class="grnum">0</div>
			</a>
		</c:if>
		<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_warning')}">
			<a class="menu i i_btn" no=text
				onclick="openNewWindow('${pageContext.request.contextPath}/views/alarm/alarmIndex.jsp')" title="预警中心" href="javascript:void(0);">
				<i class="iconfont icon-yujing"></i>
			<div id="alarmnum" class="numxl">..</div>
			</a>
		</c:if>
		<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && (fn:contains(rolePagecodeNums, 'bimPortal_bimA') || fn:contains(rolePagecodeNums, 'bimPortal_s1') || fn:contains(rolePagecodeNums, 'bimPortal_s2'))}">
			<span id="otherSystem" class="menu i i_btn show_nav_box" no=text href="javascript:void(0);" title="系统集成中心">
					...
					<div id="nav_box">
						<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_bimA')}">
						   <a id="nav1" class="one_nav" href="https://bima.hna.net/hhang_web/sys/login/BIM_home_page?vcEmployeeId=${sessionScope.DESvemployeeId}" target="_blank" title="海航实业资产管理BIM平台">
								<img src="${pageContext.request.contextPath}/img/bima.png" alt="" />
								<span>海航实业资产管理BIM平台</span>
							</a>
						</c:if>
						<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_s1')}">
							<a id="nav2" class="one_nav" href="http://omc.hnainfrastructure.com/hna_assistant/AdminMain/Index" target="_blank" title="海航基础运营管控平台">
								<img src="${pageContext.request.contextPath}/img/hn.png" alt="" />
								<span>海航基础运营管控平台</span>
							</a>
						</c:if>
						<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_s2')}">
						<a id="nav3" class="one_nav" href="http://10.71.114.21/Account/Login" target="blank" title="海航教育医疗运营管控平台">
							<img src="${pageContext.request.contextPath}/img/hhyl.png" alt="" /><span>海航教育医疗运营管控平台</span>
						</a>
						</c:if>
					</div>
			</span>
		</c:if>
		<c:if test="${fn:contains(rolePageNums, 'bimPortal_titlePage') && fn:contains(rolePagecodeNums, 'bimPortal_portal')}">
			<a class="menu i i_btn" no=text onclick="managerWin();" title="权限管理" href="javascript:void(0);">
				<i class="iconfont icon-menhuhoutai"></i>
			</a>
		</c:if>
		<%-- <span class="menu i i_btn show_nav_box" no=text href="javascript:void(0);" title="系统集成中心">
				<i class="iconfont icon-yyzx"></i>
				<div id="nav_box">
				   <a class="one_nav" href="https://bima.hna.net/hhang_web/sys/login/BIM_home_page?vcEmployeeId=${sessionScope.DESvemployeeId}" target="_blank" title="海航实业资产管理BIM平台">
						<img src="${pageContext.request.contextPath}/img/bima.png" alt="" />
						<span>海航实业资产管理BIM平台</span>
					</a>
					<a class="one_nav" href="http://omc.hnainfrastructure.com/hna_assistant/AdminMain/Index" target="_blank" title="海航基础运营管控平台">
						<img src="${pageContext.request.contextPath}/img/hn.png" alt="" />
						<span>海航基础运营管控平台</span>
					</a>
					<a class="one_nav" href="http://10.71.114.21/Account/Login" target="blank" title="海航教育医疗运营管控平台">
						<img src="${pageContext.request.contextPath}/img/hhyl.png" alt="" /><span>海航教育医疗运营管控平台</span>
					</a>
				</div>
		</span> --%>
	
		 <a class="menu i i_btn exit" id="menhuurl" href="/bim_portal"><i
	...
	<!-- <header no=text >
			...
			<div class="menu user" no="text">
				...
				<div class="avatar">
					<img src="<c:url value="http://file.hna.net/image/headimage/emp/${fn:substring(sessionScope.portal_users_session.vcEmployeeId,0,6)}0000/${sessionScope.portal_users_session.vcEmployeeId}.jpg"/>" onerror="to_def_img(this)" alt="头像">
					<img src="${pageContext.request.contextPath}/img/avatar.png" onerror="to_def_img(this)" alt="头像"> 
				</div>
			</div>
			<a class="menu i" no=text href="/bim_portal/system/ladp_portal?vcEmployeeId=${sessionScope.portal_users_session.vcEmployeeId}"><img icon src="${pageContext.request.contextPath}/img/homepage.png" alt="home"></a>
...
<map name="Map">
  ...
  <area shape="circle" coords="313,330,162" href="v.htm">
	...
	<script>
		...
		function alterNum(){
		    var employeeId='${sessionScope.portal_users_session.vcEmployeeId}';
			$.ajax({
	...
	<script>
		...
		function openzhilinwindow(){
		   // var url='${url_task}'+"/system/index?systemCode=portal&token=${token}";
		   // window.open(url, "_blank");
		   window.location.href = "${pageContext.request.contextPath}/ssoout/outsystem?url=bim_task/system/index";
		}
		
		function managerWin(){
			var url= "${pageContext.request.contextPath}/manager/_frame";
