...
<script type="text/javascript">
	function page(methed,currentPage){
		...
		if(methed==""){
			...
			url = "${url_p}"+"&pagerMethod="+methed+"&currentPage="+page+"&"+entity +"&totalPages=" +${PAGER.totalPages} ;
		}else{
			url ="${url_p}"+"&pagerMethod="+methed+"&currentPage="+currentPage+"&"+entity+"&totalPages=" +${PAGER.totalPages};
		}
...
<div class="page">
	...
	<c:if test="${PAGER.currentPage>1}">
		<div>
			<a href="#" onclick="page('first',${PAGER.currentPage})">首&nbsp;&nbsp;页</a>
			<a href="#" onclick="page('previous',${PAGER.currentPage})">上一页</a>
		</div>
	</c:if>
	<c:if test="${PAGER.totalPages>6}">
		<c:if test="${PAGER.currentPage<=6/2}">
			<c:forEach var="x" begin="1" end="6">
				...
				<c:if test="${PAGER.currentPage!=x}">
					<div class="link">
						<a href="#" onclick="page('next',${x-1})">${x}</a>
					</div>
				</c:if>
			</c:forEach>
		</c:if>
	...
	<c:if test="${PAGER.currentPage<PAGER.totalPages}">
		<div>
			<a href="#" onclick="page('next',${PAGER.currentPage})">下一页</a>
			<a href="#" onclick="page('last',${PAGER.totalPages})">尾&nbsp;&nbsp;页</a>
		</div>
	</c:if>
</div>