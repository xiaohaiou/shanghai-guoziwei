...
<body>
	...
	<div class="table_content">
		<form id="form1" action="${pageContext.request.contextPath}/sys/role/_query?type=roleCompany" method="post" modelAttribute="hhRole" class="row">
						...
						<tr>
							...
							<td>
								<c:if test="${fn:contains(rolePagecodeNums,',role_zdgssjqxfp,')==true}">
									<a class="xg" href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/sys/role/_roleCompanyAdd?id=${l.id }&pageNum=${msgPage.pageNum }&qRoleName=${qRoleName }&companyType=manage&randomId=<%=Math.random()%>')" >指定公司数据权限分配</a>
								</c:if>
								<c:if test="${fn:contains(rolePagecodeNums,',role_zdgssjqxfp,')==true}">
									<a class="xg" href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/sys/role/_roleCompanyAdd?id=${l.id }&pageNum=${msgPage.pageNum }&qRoleName=${qRoleName }&companyType=finance&randomId=<%=Math.random()%>')" >指定财务公司数据权限分配</a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
	...
	<script type="text/javascript">
		function prev(){
			...
			if(pageNum == 0){
	     	...
	     	var url = "${pageContext.request.contextPath}/sys/role/_query?type=roleCompany&pageNum="+pageNum+"&"+entity;
