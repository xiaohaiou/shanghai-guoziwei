...
　　<div class = "model_content people_model_content">
		<h4><span class="glyphicon glyphicon-user"></span> 搜索员工角色分配</h4>
			...
			<form id="form2"  method="post" action="${pageContext.request.contextPath}/sys/role/_roleUserByUserSaveOrUpdate?type=roleUser">
				...
				<div class="model_body row">
					<div class="clearboth col-md-12">
						<div class="col-md-4"><label>角色编号:</label><input type="text" value="${hhRole.roleNum }" disabled=""></div>
						<div class="col-md-4"><label>角色名称:</label><input type="text" value="${hhRole.roleName }" disabled=""></div>
						<div class="col-md-4"><label>角色描述:</label><input type="text" value="${hhRole.roleDescription }" disabled=""></div>
					</div>
					...
					<%-- 	<label class="col-md-12">人员姓名:<input type="text" id="userName" value="${userName}"><a onclick="searchUser()">搜索</a></label>  --%>
						<div class="clearboth col-md-12">
							<div class="col-md-4"><label class="col-md-12">人员姓名:<input type="text" id="userName" value="${userName}"></label></div>						
							<div class="col-md-4"><label class="col-md-12">用户账号:<input type="text" id="svcAccount" value="${vcAccount}"><a onclick="searchUser2()">搜索</a></label></div>
						</div>
...
　　</div>
	...
	/*	function searchUser(){
			...
			var url = "${pageContext.request.contextPath}/sys/role/_searchPersonByName";
		...
		function searchUser2(){
			...
			var url = "${pageContext.request.contextPath}/sys/role/_searchPersonByvcAccount";
			var vcAccount = $("#svcAccount").val();
			var userName = $("#userName").val();
			console.log($("#svcAccount"));
			console.log(vcAccount);
			console.log(userName);
			console.log((vcAccount != "" && vcAccount != null)||(userName != "" && userName != null));
			if((vcAccount != "" && vcAccount != null)||
				(userName != "" && userName != null)){
				$.ajax({
					type:"POST",
					dataType : 'json',
					data:{vcAccount:vcAccount,
						  vcEmployeeIds:vcEmployeeIds,
						  userName:userName},
					url:url,
					success:function(data){
