...
<html>
	...
	<body>
		...
		<div class="table_content">
			<form id="form1" class="row" action="${pageContext.request.contextPath}/model/user/_query" method="post">
					...
					<c:if test="${!empty requestScope.modelRegisterList }">
						...
						<c:forEach items="${ requestScope.modelRegisterList}" var="model" varStatus="status">
							<tr>
								...
								<td>
									<!-- 
									<c:if test="${fn:contains(rolePagecodeNums,',qx_ryssfp,')==true}">
										<a class="xg" href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/model/user/_modelUserAddBySearch?id=${model.id}&pageNum=${msgPage.pageNum }&qModelNum=${qModelNum }&qModelName=${qModelName }&qSysId=${qSysId }&randomId=<%=Math.random()%>')" >人员搜索分配</a>
									</c:if>
									<c:if test="${fn:contains(rolePagecodeNums,',qx_ryfp,')==true}">
										<a href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/model/user/_modelUserAdd?id=${model.id}&pageNum=${msgPage.pageNum }&qModelNum=${qModelNum }&qModelName=${qModelName }&qSysId=${qSysId }&randomId=<%=Math.random()%>')" >人员分配</a>
									</c:if> -->
									<a href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/model/user/_modelUserAdd?id=${model.id}&pageNum=${msgPage.pageNum }&qModelNum=${qModelNum }&qModelName=${qModelName }&qSysId=${qSysId }&randomId=<%=Math.random()%>')" >人员分配</a>
									<a href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/model/user/_modelUserCheck?id=${model.id}&randomId=<%=Math.random()%>')" >已分配人员查看</a>
									<a class="xg" href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/model/user/_modelUserAddBySearch?id=${model.id}&pageNum=${msgPage.pageNum }&qModelNum=${qModelNum }&qModelName=${qModelName }&qSysId=${qSysId }&randomId=<%=Math.random()%>')" >人员搜索分配</a>
	...
	<script type="text/javascript">
		...
		function prev(){
			...
			if(pageNum == 0){
	     	...
	     	var url = "${pageContext.request.contextPath}/model/user/_query?pageNum="+pageNum+"&"+entity;
