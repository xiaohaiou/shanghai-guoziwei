...
<body style="overflow:auto;">
	...
	<div class="table_content">
		<form id="form1" action="${pageContext.request.contextPath}/user/hrAndFinanceCompany/getUserFinanceCompanys" method="post" class="row">
			<div class="row">
				<span class="col-md-3">
				...
				<span class="col-md-3">
					...
					<div class=" com_ztree_box" style="display:none;height:auto;overflow:auto;max-height:300px;box;min-width:63%;position:absolute;left:6rem;background-color:#fff;z-index: 1;">
						<div>
							<input type="text" id="tempSearch" style="margin-top: 12px;margin-left: 12px;"/><a href="javascript:;" onclick="AutoMatch()" style="padding-left: 12px;">查询</a>
						</div>
...
<script type="text/javascript">
	var timeoutId
	    ...
	    function InitialZtree(){
			...
			if(tid){
				var fbNode = treeObj.getNodeByParam("vcOrganID", tid);
				if(fbNode){
		...
		function AutoMatch() {
			var txtObjvalue  = $("#tempSearch").val();
		    if (txtObjvalue && txtObjvalue.length > 0) {
		        var zTree = $.fn.zTree.getZTreeObj("com_ztree");
		        var nodes = zTree.getNodes();
				//transformToArray()此方法获取所有节点（父节点和子节点）
	    		var childrenNodes = zTree.transformToArray(nodes);
		        zTree.expandAll(false);
	    		if(childrenNodes[0]!=null)
	    			zTree.expandNode(childrenNodes[0],true);
		        var nodeList = zTree.getNodesByParamFuzzy("name", txtObjvalue);
		        console.log(nodeList);
		        if(nodeList && nodeList.length >0){
		        	
	        		var fbNode = nodeList[0];
		        	if(fbNode){
						expandParent(zTree, fbNode);
						zTree.selectNode(fbNode);
					}
		        }
		    } 
		}
		...
		*/
		function expandParent(treeObj,fbNode){
			var pfbNode = treeObj.getNodeByParam("vcOrganID", fbNode.pId);
			if(pfbNode != null){
		...
		$("#com_ztree").click(function(){
				setTimeout(function(){
	   				if($.fn.zTree.getZTreeObj("com_ztree").getSelectedNodes()[0])
						{
							 $("#vcOrganId").val($.fn.zTree.getZTreeObj("com_ztree").getSelectedNodes()[0].vcOrganID)
		...
		function export_1(){
			...
			form.action = "${pageContext.request.contextPath}/user/hrAndFinanceCompany/_export2";
			form.method = "post";
			form.submit();
		}
		...
		function query(){
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/user/hrAndFinanceCompany/getUserFinanceCompanys";
			form.method = "post";
			form.submit();
		}
</script>
