			...
			<form id="form2"  method="post" >
				...
				<div class="model_body row">
					<div class="clearboth col-md-12" style="margin-top: 20px;">
						<div class="col-md-4"><label>模块编号:</label><input type="text" value="${registerModel.modelNum }" disabled=""></div>
						<div class="col-md-4"><label>模块名称:</label><input type="text" value="${registerModel.modelName }" disabled=""></div>
						<div class="col-md-4"><label>模块描述:</label><input type="text" value="${registerModel.modelDescription }" disabled=""></div>
					</div>
					<div class="row" id="">
						...
						<div class="people_select_list" style="font-size: 17px;width: 98%;">
		  <div class="tablebox">
				<table>
					...
					<c:if test="${!empty requestScope.modelPersonList }">
						<c:set var="recordNumber" value="${(msgPage.pageNum - 1)*10 }"></c:set>
						<c:forEach items="${ requestScope.modelPersonList}" var="person" varStatus="status">
							<tr>
								<td>
...
　　</div>
	...
	<script type="text/javascript">
		function submitUsersModelBySearch(){
		    ...
		    form.action="${pageContext.request.contextPath}/model/register/modelPersonQuery"
	       form.submit();
		}
		function exportPerson(){
			var form = document.forms[0];
		form.action = "${pageContext.request.contextPath}/model/register/modelPersonExport";
		form.method = "post";
		form.submit();
		}
		//关闭弹窗
