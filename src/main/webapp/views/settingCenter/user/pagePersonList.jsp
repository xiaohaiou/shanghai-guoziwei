		...
		<div style="overflow:hiddens;margin-bottom: 35px;margin-top: -10px;"  class="model_body model_shadow">
			<form id="form2"  method="post" >
				...
				<div class="model_body row">
					<div class="clearboth col-md-12" style="margin-top: 20px;">
						<div class="col-md-4"><label>页面编号:</label><input type="text" value="${registerModel.pageNum }" disabled=""></div>
						<div class="col-md-4"><label>页面名称:</label><input type="text" value="${registerModel.pageName }" disabled=""></div>
						<div class="col-md-4"><label>页面描述:</label><input type="text" value="${registerModel.pageDescription }" disabled=""></div>
					</div>
					<div class=" row" id="">
		  ...
		  <div class="tablebox">
				<table>
					...
					<c:if test="${!empty requestScope.PagePersonList }">
						<c:set var="recordNumber" value="${(msgPage.pageNum - 1)*10 }"></c:set>
						<c:forEach items="${ requestScope.PagePersonList}" var="person" varStatus="status">
							<tr>
								<td>
...
　　</div>
	...
	<script type="text/javascript">
		...
		function exportPerson(){
			var form = document.forms[0];
		form.action = "${pageContext.request.contextPath}/page/register/pagePersonExport";
		form.method = "post";
		form.submit();
		}
		//关闭弹窗
