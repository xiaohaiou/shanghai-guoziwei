...
<html>
	...
	<body>
		...
		<div class="table_content">
			<form id="form1" class="row" action="${pageContext.request.contextPath}/page/register/_query" method="post">
				...
				<div class="form_div col-md-3">
					...
					<c:if test="${fn:contains(rolePagecodeNums,',page_add,')==true}">
						<input type="button" value="新增" class="form_btn" id="model_add" onclick="openDialog('${pageContext.request.contextPath}/page/register/_addOrModify?op=add&pageNums=${msgPage.pageNum }&qPageNum=${qPageNum }&qPageName=${qPageName }&qModelId=${qModelId }')">
					</c:if> -->
					<input type="button" value="新增" class="form_btn" id="model_add" onclick="openDialog('${pageContext.request.contextPath}/page/register/_addOrModify?op=add&pageNums=${msgPage.pageNum }&qPageNum=${qPageNum }&qPageName=${qPageName }&qModelId=${qModelId }')">
			...
			<div class="tablebox">
							...
							<tr>
								...
								<td>
									...
									<c:if test="${fn:contains(rolePagecodeNums,',page_edit,')==true}">
										<a href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/page/register/_addOrModify?op=modify&id=${page.id}&pageNums=${msgPage.pageNum }&qPageNum=${qPageNum }&qPageName=${qPageName }&qModelId=${qModelId }&randomId=<%=Math.random()%>')" >编辑</a>
									</c:if>
									<c:if test="${fn:contains(rolePagecodeNums,',page_del,')==true}">
										<a href="javascript:;" onclick="del(${page.id})">删除</a>
									</c:if>
									<c:if test="${fn:contains(rolePagecodeNums,',page_show,')==true}">
										<a href="javascript:;" onclick="show(${page.id})">查看人员</a>
									</c:if> -->
									<a href="javascript:void(0)" onclick="openDialog('${pageContext.request.contextPath}/page/register/_addOrModify?op=modify&id=${page.id}&pageNums=${msgPage.pageNum }&qPageNum=${qPageNum }&qPageName=${qPageName }&qModelId=${qModelId }&randomId=<%=Math.random()%>')" >编辑</a>
									<a href="javascript:;" onclick="del(${page.id})">删除</a>
									<a href="javascript:;" onclick="show(${page.id})">查看人员</a>
				...
				<input type="hidden" id="pageNums" name="pageNums" value="${msgPage.pageNum }">
	...
	<script type="text/javascript">
		...
		function prev(){
			...
			var params = {};
			var tempPageNum = document.getElementById("pageNums").value; 
			...
			if(pageNum == 0){
	     	...
	     	var url = "${pageContext.request.contextPath}/page/register/_query?pageNums="+pageNum+"&"+entity;
		...
		function del(id){
			parent.win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/page/register/_del?id=" + id;
					$.post(url, function(data) {
						if(data == 'success'){
							parent.win.successAlert('删除成功！');
						}else{
							parent.win.successAlert('此页面有注册的功能未删除，请先删除此页面注册的功能！删除失败！');	
						}
						$("#form1").submit();
					});
				}
		...
		function show(id){
			var url = "${pageContext.request.contextPath}/page/register/PagePersonQuery?modelId=" + id;
			mload(url);
		}
