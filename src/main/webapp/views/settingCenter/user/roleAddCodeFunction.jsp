...
<html>
	<head>
		...
		<style>
			...
			/* 弹出框样式 */
			.model_t1,.model_t2,.model_t3,.model_t4{
				display:block;
				vertical-align: bottom;
				font-weight:bold
			}
			.model_t1>span,.model_t2>span,.model_t3>span,.model_t4>span{
				color:#666;
				font-size:14px;
				font-weight: normal;
			}
...
　　		<div class = "model_content">
			<h4><span class="glyphicon glyphicon-pencil"></span>功能选择</h4>
			
			<div style="overflow:scroll;overflow-x:hidden" class="model_body">
				<form id="form2" action="${pageContext.request.contextPath}/sys/role/_saveRoleCodeFunction?type=roleFunction" method="post">
					...
					<div class="row">
						<label class="col-xs-12">功能选择：</label>
	...
	<script type="text/javascript">
		...
		$("[name=chkType]").on("click",function(e){
			
			/* $(this).parent().next().find(".model_t4 .char_label").click();
			$(this).parent().next().find("ul li .char_label").click(); */
			
			$(this).parent().next().find(".char_label").click();
			
		});
		/* $("[name=chkPage]").on("click",function(){
