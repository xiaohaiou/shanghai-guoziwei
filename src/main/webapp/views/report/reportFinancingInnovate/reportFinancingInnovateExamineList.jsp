...
<html>
	...
	<!-- 		<span class="col-md-4">
		 ...
		 -->	
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_cxlrzxzSh_q,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_cxlrzxzSh_k_e,')==true }">	
					<input type="button" value="一键审核" class="form_btn" id="qrybtn" onclick="keyExmine()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_cxlrzxzSh_hzcx,')==true }">
					<a  href="${mapurl}/g_query"  target="_blank"><input type="button" value="汇总查询" background-color="#3399CC"/></a>
				</c:if>
			</div>
		</form>
			<div class="tablebox">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${sys.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_cxlrzxzSh_e,')==true }">	
	...
	function checkdata(url,id)
		...
		function keyExmine(){
			var form = document.forms[0];
			form.action = "/shanghai-gzw/reportFinancingInnovate/keyExmine";
			form.method = "post";
			form.submit();	
		}
