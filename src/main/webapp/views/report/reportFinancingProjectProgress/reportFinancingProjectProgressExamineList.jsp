...
<html>
	...
	<body>
		...
		<!-- 	<span class="col-md-4">
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_yblrzxzSh_q,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_yblrzxzSh_k_e,')==true }">	
					<input type="button" value="一键审核" class="form_btn" id="keyexminebtn" onclick="keyExmine()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_yblrzxzSh_hzcx,')==true }">
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									<fmt:parseDate value="${sys.expectAccountDate }" var="date" pattern="yyyy-MM-dd"/>  
								...
								<td>
									...
									<c:if test="${sys.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_yblrzxzSh_e,')==true }">	
	...
	function checkdata(url,id)
		...
		function keyExmine(){
			var form = document.forms[0];
			form.action = "/shanghai-gzw/reportFinancingProjectProgress/keyAudit";
			form.method = "post";
			form.submit();		
		}
