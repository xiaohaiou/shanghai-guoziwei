...
<body>
	<h4>
		<span class="glyphicon glyphicon-pencil"></span>审核融资项目进展数据
	...
	<div class="label_inpt_div">
		<form:form id="form2" modelAttribute="entityview" >
			...
			<div class="model_part">
				...
				<label class="w20" style="vertical-align:top;">条件及结构:</label>
				<label class="w75 setleft" style="word-wrap:break-word;">${ entityview.conditionStructure}</label>
				...
				<label class="w20">手续费率(%):</label>
				<label class="w25 setleft">${ entityview.serviceChargeRatio}</label>
				<label class="w20">综合融资成本(%):</label>
				<label class="w25 setleft">${ entityview.comprehensiveFinancingCostRatio}</label>
				<label class="w20"></label>
				<label class="w25 setleft"></label>
				<label class="w20" style="vertical-align:top;">项目进展描述:</label>
				<label class="w75 setleft" style="word-wrap:break-word;">${ entityview.projectProgressDescribe}</label>
				<label class="w20" style="vertical-align:top;">项目后续推进:</label>
				<label class="w75 setleft" style="word-wrap:break-word;">${ entityview.projectFollowupAdvance}</label>
				<label class="w20" style="vertical-align:top;">碰到的问题:</label>
				<label class="w75 setleft" style="word-wrap:break-word;">${ entityview.problem}</label>
				<label class="w20" style="vertical-align:top;">项目难点:</label>
				<label class="w75 setleft" style="word-wrap:break-word;">${ entityview.projectDifficulty}</label>
				<label class="w20" style="vertical-align:top;">解决方式:</label>
				<label class="w75 setleft" style="word-wrap:break-word;">${ entityview.solution}</label>
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/reportFinancingProjectProgress/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/reportFinancingProjectProgress/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
		    		var commitresult=JSON.parse(data);
							if(commitresult.result==true)
