...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil">修改融资项目进展数据</span>
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增融资项目进展数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	...
	<div class="label_inpt_div">
		<form:form id="form2" modelAttribute="entityview" >
		 ...
		 <!--  <label class="w20"><span class="required"> * </span>单位名称:</label>   
				<span class="w25">
					<select  name="coreOrg" id="coreOrg" class="" onchange="getCoreOrgname()">
						<option value=""  ></option>
				...
				<label class="w20"><span class="required"> * </span>类别:</label>
				<span class="w25">
				...
				<label class="w20"><span class="required"> * </span>操作主体:</label>
				<form:input class="w25" path="operateOrgname" id="operateOrgname" check="NotEmpty_string_._._._._."/>
				<%-- <span class="w25">
						<input type="hidden" id="operateOrg" name="operateOrg" value="${entityview.operateOrg }" >
						<input name="operateOrgname" id="operateOrgname" value="${entityview.operateOrgname }" check="NotEmpty_string_._._._._." readonly="true">
						<div class=" com_ztree_box" style="display:none;height:auto;overflow:auto;max-height:300px;box;min-width:63%;position:absolute;left:6rem;background-color:#fff;z-index: 1;">
							<ul id="operate_com_ztree" class="ztree">
							</ul>
					   </div>
				</span> --%>
				<label class="w20"><span class="required"> * </span>融资主体:</label>
				<form:input class="w25" path="financingOrgname" id="financingOrgname" check="NotEmpty_string_._._._._."/>
				<%-- <span class="w25">
						<input type="hidden" id="financingOrg" name="financingOrg" value="${entityview.financingOrg }" >
						<input name="financingOrgname" id="financingOrgname" value="${entityview.financingOrgname }" check="NotEmpty_string_._._._._." readonly="true">
						<div class=" com_ztree_box" style="display:none;height:auto;overflow:auto;max-height:300px;box;min-width:63%;position:absolute;left:6rem;background-color:#fff;z-index: 1;">
							<ul id="financing_com_ztree" class="ztree">
							</ul>
					   </div>
				</span> --%>
				...
				<label class="w20"><span class="required"> * </span>金融机构:</label>
				...
				<label class="w20"><span class="required"> * </span>新或续:</label>
				<span class="w25">
				...
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>条件及结构:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="conditionStructure" id="conditionStructure" >${entityview.conditionStructure}</textarea></span>
				<label class="w20"><span class="required"> * </span>已下账金额(亿元):</label>
				<form:input class="w25" path="alreadyAccountAmounts" id="alreadyAccountAmounts" check="NotEmpty_double_16_4_0+_._."/>
				...
				<label class="w20"><span class="required">  </span>利率(%):</label>
				<form:input class="w25" path="interestRatio" id="interestRatio"  /><%-- check="NotEmpty_double_6_2_0+_._." --%>
				<label class="w20"><span class="required"> * </span>手续费率(%):</label>
				<form:input class="w25" path="serviceChargeRatio" id="serviceChargeRatio" check="NotEmpty_double_6_2_0+_._."/>
				<label class="w20"><span class="required"> * </span>综合融资成本:</label>
				<form:input class="w25" path="comprehensiveFinancingCostRatio" id="comprehensiveFinancingCostRatio" check="NotEmpty_string_._._._._."/><%-- check="NotEmpty_double_6_2_0+_._." --%>
				<label class="w20"></label>
				<label class="w25 setleft"></label> 
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>项目进展描述:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="projectProgressDescribe" id="projectProgressDescribe" >${entityview.projectProgressDescribe}</textarea></span>
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>项目后续推进:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="projectFollowupAdvance" id="projectFollowupAdvance">${entityview.projectFollowupAdvance}</textarea></span>
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>碰到的问题:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="problem" id="problem" >${entityview.problem}</textarea></span>
				<label class="w20" style="vertical-align: top;">项目难点:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="projectDifficulty" id="projectDifficulty">${entityview.projectDifficulty}</textarea></span>
				<label class="w20" style="vertical-align: top;">解决方式:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="solution" id="solution">${entityview.solution}</textarea></span>
			</div>
...
<script type="text/javascript">
		...
		{
			$("#coreOrg").val($("#coreOrg").find("option:selected").text());
		}
		...
		function commitcheck()
		{
			...
			if($("#conditionStructure").val()=="")
			{
				win.errorAlert("条件及结构不能为空");
				return false;
			}
			if($("#projectProgressDescribe").val()=="")
			{
				win.errorAlert("项目进展描述不能为空");
				return false;
			}
			if($("#projectFollowupAdvance").val()=="")
			{
				win.errorAlert("项目后续推进不能为空");
				return false;
			}
			if($("#problem").val()=="")
			{
				win.errorAlert("碰到的问题不能为空");
				return false;
			}
			return true;
		}
		...
		$("#commit-1").click(function(){
			...
			var url = "${pageContext.request.contextPath}/reportFinancingProjectProgress/save";
		...
		$("#commit-2").click(function(){
			...
			if(!vaild.all() || !commitcheck())
			{
					...
					//var entityBasicInfo = $('#form2').serialize();
			var url = "${pageContext.request.contextPath}/reportFinancingProjectProgress/saveandreport";
			var formData = new FormData($("#form2")[0]);
			$.ajax({  
			     url : url,  
			     type : "POST",  
