	...
	<body>
		...
		<form id="form1"  class="row">
			...
			<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_add,')==true }">	
						<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_dataExport,')==true}">
						<input type="button" value="导出" class="form_btn" id="exportbtn" onclick="_export()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_export,')==true }">	
						<input type="button" value="个人借款台账表格下载" class="form_btn" id="export" onclick="exportfile()">
						<a href="${pageContext.request.contextPath}/views/report/reportPersonalLoanNew/personalloan.xlsx" id="exportfile" style="display:none"></a>
					</c:if>
							...
							<tr>
								...
								<td>
									...
									<c:if test="${sys.status==50114 || sys.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_recheck,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">复核</a>
										</c:if>	
										<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_modify,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${sys.id}',${sys.id })">复核</a>
										</c:if>	
										<c:if test="${fn:contains(gzwsession_code,',bimWork_grjksjtb_delete,')==true  }">	
