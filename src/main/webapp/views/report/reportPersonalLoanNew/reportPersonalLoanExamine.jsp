...
<body>
			...
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>审核个人借款数据
...
<script type="text/javascript">
		function initNodeList(v,pageNums){
			$.ajax({
				url:"${pageContext.request.contextPath}/reportpersonalLoanNew/_itemViewList?pageNums="+pageNums+"&id=${entityview.id}",
				type:"POST",
				dataType:"text",
				async:false,
		...
		$("#commit-1").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/reportpersonalLoanNew/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/reportpersonalLoanNew/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
		    		var commitresult=JSON.parse(data);
							if(commitresult.result==true)
