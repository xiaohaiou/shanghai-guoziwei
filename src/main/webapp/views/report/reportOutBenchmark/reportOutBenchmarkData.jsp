			...
			<form id="form1"  class="row"  >
         	...
         	<%-- 		<span class="col-md-4">
				...
				<div class="form_div col-md-12">
            			<c:if test="${fn:contains(gzwsession_code,',bimWork_wbdbsjtb_query,')==true }">	 
						    <input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
 					    </c:if>
					    <c:if test="${fn:contains(gzwsession_code,',bimWork_wbdbsjtb_new,')==true }">	 
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_wbdbsjtb_show,')==true }">	 
									     <a href="javascript:void(0)" onclick="mload('${mapurl}/view?op=show&id=${sys.id}')" >查看</a>
								    </c:if> 
									<c:if test="${sys.status==50114 || sys.status==50112 }">
									     <c:if test="${fn:contains(gzwsession_code,',bimWork_wbdbsjtb_modify,')==true }">	
									          <a href="javascript:void(0)" onclick="mload('${mapurl}/newmodify?op=modify&id=${sys.id}')" >维护</a>
									     </c:if>
									     <c:if test="${fn:contains(gzwsession_code,',bimWork_wbdbsjtb_report,')==true }">	
									          <a href="javascript:void(0)" onclick="mload('${mapurl}/postexam?op=report&id=${sys.id}')" >上报</a>
									     </c:if>
									     <c:if test="${fn:contains(gzwsession_code,',bimWork_wbdbsjtb_delete,')==true }">	
