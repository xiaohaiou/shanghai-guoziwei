...
<html>
		...
		<input type="hidden" name="groupType" value="${grouptype }">
		   ...
		   <span class="col-md-4">
			...
			<span class="col-md-4">
				...
				<input  type="text" id="starttime" name="starttime" value="${entityview.starttime }" readonly="true" class="time time_short" />
				至
				<input  type="text" id="endtime" name="endtime" value="${entityview.endtime }" readonly="true" class="time time_short" />
			</span>
			
			<span class="col-md-12 " style="text-align: right">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_query,')==true }">
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_new,')==true }">
						<input type="button" value="数据导入" class="form_btn" id="addbtn" onclick="mload('${pageContext.request.contextPath}/reportcommon/newmodify?op=new&type=${type}&grouptype=${grouptype}')">
						
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_exportfile,')==true }">
						<c:if test="${! empty templet.filePath }">
							<input type="button" class="form_btn" value="模板导出" onclick="exportout()"></a>
						</c:if>
					</c:if>
			</span>
		</form>
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status1">
							<tr>
								...
								<td>
								<%-- checkdata="${sys.groupID},${sys.organalID },${sys.time}" --%>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_query,')==true }">
									...
									<c:if test="${sys.examinestatus==50115}">
										<a href="javascript:;" onclick="downloadFile('${pageContext.request.contextPath}/reportcommon/exportReportData?type=${type}&groupID=${sys.groupID}&organal=${sys.organalID}&Date=${sys.time}&grouptype=${grouptype }')"  checkdata="${sys.groupID},${sys.organalID },${sys.time}">导出</a>
									</c:if>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_edit,')==true }">
									<c:if test="${sys.examinestatus==50114 || sys.examinestatus==50112 }">
										<a href="javascript:void(0)"  onclick="mload('${pageContext.request.contextPath}/reportcommon/newmodify?op=modify&type=${type }&groupID=${sys.groupID}&organalID=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }')">修改</a>
									</c:if>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_report,')==true }">
									<c:if test="${sys.examinestatus!=50113 && sys.examinestatus!=50115 }">
									<a  checkdata="${sys.groupID},${sys.organalID },${sys.time}"  href="${pageContext.request.contextPath}/reportcommon/show?op=report&type=${type }&groupID=${sys.groupID}&organal=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }" target="_blank">上报</a>
									</c:if>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbsb_delete,')==true }">
									<c:if test="${sys.examinestatus==50114 || sys.examinestatus==50112 }">
										<a href="javascript:void(0)" onclick="del('${sys.groupID}','${sys.organalID}','${sys.time }')" >删除</a>
									</c:if>
								</c:if>	
								</td>
							</tr>
	...
	<script type="text/javascript">
		...
		function del(groupid,organalid,date){
			win.confirm('确定要删除？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/reportcommon/delete?Date=" + date+"&organalID="+organalid+"&groupid="+groupid;
