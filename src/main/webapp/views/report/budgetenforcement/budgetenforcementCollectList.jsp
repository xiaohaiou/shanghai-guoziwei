	...
	<body>
		...
		<input type="hidden" name="groupType" value="${grouptype }">
			...
			<span class="col-md-12 " style="text-align: right">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbhzsb_query,')==true }">
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbhzsb_new,')==true }">
						<input type="button" value="汇总生成" class="form_btn" id="addbtn" onclick="mload('${pageContext.request.contextPath}/reportbudgetenforcementcollect/newmodify?op=new&grouptype=${grouptype }')">
					</c:if>
			</span>
		</form>
			<div class="tablebox">
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status1">
							<tr>
								...
								<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbhzsb_report,')==true }">
								...
								<c:if test="${fn:contains(gzwsession_code,',bimWork_yszxbbhzsb_delete,')==true }">
