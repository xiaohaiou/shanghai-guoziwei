...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>修改本周融资项目进展数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增本周融资项目进展数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	...
	<div class="label_inpt_div">
		<form:form id="form2" modelAttribute="entityview" >
			...
			<h3 class="data_title1">本周下账清单：</h3>
			<div class="model_part">
				<div class="tablebox">
					<table id="bzqdTableId">
						...
						<c:forEach items="${entityview.weekThisSet}" var="result" varStatus="s">
							<tr id="bzqdTr${result.fid}">
								...
								<td>${result.lastModifyDate}
									<input type="hidden" name="weekThisList[${ s.index }].fid" value ="${result.fid}">
									<input type="hidden" name="weekThisList[${ s.index }].financialInstitution" value = "${result.financialInstitution}">
									<input type="hidden" name="weekThisList[${ s.index }].operateOrgname" value = "${result.operateOrgname}">
									<input type="hidden" name="weekThisList[${ s.index }].operateOrg" value = "${result.operateOrg}">
									<input type="hidden" id="aaaInputId${result.fid}" name="weekThisList[${ s.index }].alreadyAccountAmounts" value ="${result.alreadyAccountAmounts}">
									<input type="hidden" id="adInputId${result.fid}" name="weekThisList[${ s.index }].accountDate" value = "${result.accountDate}">
									...
									<input type="hidden" name="weekThisList[${ s.index }].lastModifyDate" value = "${result.lastModifyDate}">	
								</td>
...
<script type="text/javascript">
		...
		function selData(){
			var url = "${pageContext.request.contextPath}/reportFinancingWeekThis/getAccountsData";
		...
		function addTableHtml(trNum,data,flag){
		    ...
		    if(data!=null&&flag==2){
				...
				tr+="<td><input type='button' value='加入本周下账清单' class='form_btn' onclick='subWeekThis("+data[7]+")'></td></tr>";//加入本周下账清单
				...
				if(fidArray.indexOf("&bzqdTr"+trNum+"&")<0){
					...
					tr+="<input type='hidden' name='weekThisList["+listNum+"].fid' value ='"+data[7]+"'>";
					tr+="<input type='hidden' name='weekThisList["+listNum+"].financialInstitution' value = '"+data[0]+"'>";
					tr+="<input type='hidden' name='weekThisList["+listNum+"].operateOrgname' value = '"+data[2]+"'>";
					tr+="<input type='hidden' name='weekThisList["+listNum+"].operateOrg' value = '"+data[1]+"'>";
					tr+="<input type='hidden' id='aaaInputId"+trNum+"' name='weekThisList["+listNum+"].alreadyAccountAmounts' value ='"+data[3]+"'>";
					tr+="<input type='hidden' id='adInputId"+trNum+"' name='weekThisList["+listNum+"].accountDate' value = '"+data[4]+"'>";
					tr+="<input type='hidden' name='weekThisList["+listNum+"].projectProgress' value = '"+data[8]+"'>";
					tr+="<input type='hidden' name='weekThisList["+listNum+"].lastModifyDate' value = '"+(data[6]==null?data[9]:data[6])+"'></tr>";
		...
		*/
		...
		$("#commit-1").click(function(){
			...
			var url = "${pageContext.request.contextPath}/reportFinancingWeekThis/save";
		...
		$("#commit-2").click(function(){
			...
			if(!vaild.all() || !commitcheck())
			{
					...
					//var entityBasicInfo = $('#form2').serialize();
			var url = "${pageContext.request.contextPath}/reportFinancingWeekThis/saveandreport";
			var formData = new FormData($("#form2")[0]);
			$.ajax({  
			     url : url,  
			     type : "POST",  
