...
<html>
	...
	<body>
		...
		<%-- 	<span class="col-md-4">
			...
			<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjsh_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjsh_export,')==true }">	
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${sys.status==50113 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjsh_exam,')==true }">	
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/exam?id=${sys.id}',${sys.id })" >审核</a>
										</c:if>
									</c:if>
									<c:if test="${sys.status==50115 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjsh_return,')==true }">
	...
	/* 
	  ...
	  */
		...
		function _export(){
			var form = document.forms[0];
				form.action = "${pageContext.request.contextPath}/reportOverseasAsset/export";
				form.method = "post";
				form.submit();
		}
		function del(id){
