...
<html>
	...
	<body>
		...
		<%-- 	<span class="col-md-4">
			...
			<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjtb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjtb_new,')==true }">	
						<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjtb_export,')==true }">	
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.outasset }"  pattern="#,##0.0000" />亿元
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.insideasset }"  pattern="#,##0.0000" />亿元
								</td>
								<td style="text-align: right">
									<c:if test="${!empty sys.insideasset }">
										<fmt:formatNumber value="${100*sys.outasset/sys.insideasset}"  pattern="#,##0.00" />%
									</c:if>
								</td>
								...
								<td>
									...
									<c:if test="${sys.status==50114 || sys.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjtb_edit,')==true }">	
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${sys.id}',${sys.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjtb_report,')==true }">	
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jwzcbsjtb_delete,')==true }">
	...
	/* 
	  ...
	  */
		...
		function _export(){
		var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/reportOverseasAsset/reportExport";
			form.method = "post";
			form.submit();
		}
