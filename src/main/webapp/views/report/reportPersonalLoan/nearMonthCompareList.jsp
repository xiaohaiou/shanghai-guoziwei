...
<html>
		...
		<form id="form1"  class="row">
			...
			<div class="tablebox">
				...
				<table>
					...
					<c:if test="${!empty requestScope.msgPage.list[0] }">
					  <c:set var="recordNumber1" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list[0].detaillist}" var="sys" varStatus="status">
							<tr>
								<td>
	...
	var parentID = '${entityview.coreorg}';
	if(parentID != ''){
		$.ajax({
			url:"${pageContext.request.contextPath}/reportpersonalLoan/getcompanyName",
			type:"POST",
			async:false,
