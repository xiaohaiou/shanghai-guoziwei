	...
	<body>
		...
		<form id="form1"  class="row">
			...
			<div class="form_div col-md-12">
				<c:if test="${entityview.type ==0}">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjtb_nb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjtb_nb_add,')==true }">	
						<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify?type='+${entityview.type}+'')">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjtb_nb_export,')==true }">	
						<input type="button" value="应收债权表格下载" class="form_btn" id="export" onclick="exportfile()">
						<a href="${pageContext.request.contextPath}/views/report/reportReceivabledebt/receivabledebt.xlsx" id="exportfile" style="display:none"></a>
					</c:if>
				</c:if>	
				<c:if test="${entityview.type ==1}">
					...
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjtb_wb_export,')==true }">	
						<input type="button" value="应收债权表格下载" class="form_btn" id="export" onclick="exportfile()">
						<a href="${pageContext.request.contextPath}/views/report/reportReceivabledebt/receivabledebt1.xlsx" id="exportfile" style="display:none"></a>
					</c:if>
				</c:if>
							...
							<tr>
								...
								<td>
									<c:if test="${entityview.type ==0}">
										...
										<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjtb_nb_recheck,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">复核</a>
										</c:if>	
										<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjtb_nb_del,')==true  }">	
