	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">
				<c:if test="${entityview.type ==0}">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjsh_nb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
				</c:if>
				<c:if test="${entityview.type ==1}">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjsh_wb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
				</c:if>
							...
							<tr>
								...
								<td>
									<c:if test="${entityview.type ==0}">
										...
										<c:if test="${sys.status==50113 }">
											<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjsh_nb_exam,')==true }">	
									...
									<c:if test="${entityview.type ==1}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjsh_wb_exam,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=modify&id=${sys.id}',${sys.id})" >查看</a>
										</c:if>
										<c:if test="${sys.status==50113 }">
											<c:if test="${fn:contains(gzwsession_code,',bimWork_yszqsjsh_wb_exam,')==true }">	
