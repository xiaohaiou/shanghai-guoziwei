	...
	<c:choose>
		<c:when test="${op eq 'report'}">
			...
			<c:if test="${entityview.type ==0}">
				<span class="glyphicon glyphicon-pencil"></span>上报应收债权(内部)台账信息
			</c:if>
		...
		<c:otherwise>
			<h4>
			<c:if test="${entityview.type ==0}">
				<span class="glyphicon glyphicon-pencil"></span>查看应收债权(内部)台账信息
			</c:if>
			<c:if test="${entityview.type ==1}">
...
<script type="text/javascript">
		...
		function exam(id){	
			...
			var url = "/shanghai-gzw/receivabledebt/recheck?id=" + id;
