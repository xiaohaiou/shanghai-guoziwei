	...
	<c:choose>
		<c:when test="${op eq 'modify'}">
			...
			<c:if test="${entityview.type ==0}">
				<span class="glyphicon glyphicon-pencil"></span>修改应收债权(内部)台账数据
			</c:if>	
		...
		<c:otherwise>
			<h4>
			<c:if test="${entityview.type ==0}">
				<span class="glyphicon glyphicon-pencil"></span>应收债权(内部)台账数据导入
			</c:if>	
			<c:if test="${entityview.type ==1}">
		...
		<form:form id="form2" modelAttribute="entityview" >
				...
				<form:hidden path="id"/>
				<input type="hidden" id="type" name="type" value="${entityview.type}" >
				<div class="label_inpt_div">
				<div class="model_part">
				
				<label class="w20"><span class="required"> * </span>上报单位:</label>
				<span class="w25">
					<input type="hidden" id="debtorg" name="debtorg" value="${entityview.debtorg }" >
						<input name="debtorgname" id="debtorgname" value="${entityview.debtorgname }" check="NotEmpty_string_._._._._." readonly>
				...
				<c:if test="${entityview.type ==0}">
					<label class="w20">应收债权(内部)台账表:</label>
				</c:if>
				<c:if test="${entityview.type ==1}">
...
<script type="text/javascript">
		...
		$("#commit-1").click(function(){
			...
			var url = "${pageContext.request.contextPath}/receivabledebt/save";
