...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>修改共享类融资项目进展数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增共享类融资项目进展数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	...
	<div class="label_inpt_div">
		<form:form id="form2" modelAttribute="entityview" >
			...
			<!--  	<label class="w20"><span class="required"> * </span>单位名称:</label>
				...
				<label class="w20"><span class="required"> * </span>前期/顾问费(%):</label>
				<form:input class="w25" path="preConsultantFee" id="preConsultantFee" check="NotEmpty_int_16_._0+_._."/>
				...
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>用途介绍:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="purposeToIntroduce" id="purposeToIntroduce" >${entityview.purposeToIntroduce}</textarea></span>
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>具体进展:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="specificProgress" id="specificProgress" >${entityview.specificProgress}</textarea></span>
				<label class="w20" style="vertical-align: top;"><span class="required"> * </span>具体需求及建议:</label>
				<span class="w70 "><textarea  rows="5" cols="50" name="spSuggestions" id="spSuggestions" >${entityview.spSuggestions}</textarea></span>
...
<script type="text/javascript">
	...
	$("#operate_com_ztree").click(function(){
	   				...
	   				if($.fn.zTree.getZTreeObj("com_ztree").getSelectedNodes()[0])
						{
							...
							var url = "${pageContext.request.contextPath}/reportFinancingShare/isVirtualCompany";
							$.ajax({  
							     url : url,  
							     type : "POST",  
		...
		function commitcheck()
		{
			...
			if($("#purposeToIntroduce").val()=="")
			{
				win.errorAlert("用途介绍信息不能为空");
				return false;
			}
			if($("#specificProgress").val()=="")
			{
				win.errorAlert("具体进展信息不能为空");
				return false;
			}
			if($("#spSuggestions").val()=="")
			{
				win.errorAlert("具体需求及建议不能为空");
				return false;
			}
			return true;
		}
		...
		$("#commit-1").click(function(){
			...
			var url = "${pageContext.request.contextPath}/reportFinancingShare/save";
		...
		$("#commit-2").click(function(){
			...
			if(!vaild.all() || !commitcheck())
			{
					...
					//var entityBasicInfo = $('#form2').serialize();
			var url = "${pageContext.request.contextPath}/reportFinancingShare/saveandreport";
			var formData = new FormData($("#form2")[0]);
			$.ajax({  
			     url : url,  
			     type : "POST",  
