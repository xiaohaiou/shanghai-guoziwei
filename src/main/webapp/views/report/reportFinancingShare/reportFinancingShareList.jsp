		...
		<!-- 	<span class="col-md-4">
			...
			<span class="col-md-4">
				<span class="search_title" style="width:9em">洽谈金融机构：</span>
					<input type="text" id="financialInstitution" name="financialInstitution" value="${entityview.financialInstitution }" style="width: calc(100% - 10em);">
			</span>
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_q,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_n,')==true }">	
					<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_dc,')==true }">
					<input type="button" value="导出" class="form_btn" id="addbtn" onclick="_export(1)">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_hzcx,')==true }">
							...
							<tr>
								...
								<td>
									...
									<c:if test="${sys.status==50114 || sys.status==50112 || sys.status==50115}">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_e,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${sys.id}',${sys.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_qr,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_gxlrzxzTb_d,')==true && sys.status!=50115}">
