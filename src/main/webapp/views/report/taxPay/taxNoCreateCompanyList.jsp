...
<html>
	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<span class="col-md-12" style="text-align: right">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">			
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_export,')==true }">	
				     <input type="button" value="导出" class="form_btn" id="exportbtn" onclick="_export()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_remind,')==true }">	<!-- bimWork_swwtbgscx_remind -->
					<input type="button" value="填报提醒" class="form_btn" id="qrybtn" onclick="_remind()">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_remind_all,')==true }">	<!-- bimWork_swwtbgscx_remind_all -->
					<input type="button" value="全选" class="form_btn" id="qrybtn" onclick="_addRemindAll()">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_disRemind_all,')==true }">	<!-- bimWork_swwtbgscx_disRemind_all -->
					...
					<c:if test="${not empty requestScope.msgPage.list}">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status1">
							<tr>
								...
								<td style="text-align: center">
									<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_remind_add_temp,')==true }">	<!-- bimWork_swwtbgscx_remind_add_temp -->
										<input type="button" value="+" class="form_btn" id="qrybtn" onclick="_addRemind('${sys[3]}','1')">
									</c:if>	
									<c:if test="${fn:contains(gzwsession_code,',bimWork_swwtbgscx_remind_remove_temp,')==true }">	<!-- bimWork_swwtbgscx_remind_remove_temp-->
										<input type="button" value="-" class="form_btn" id="qrybtn" onclick="_removeRemind('${sys[3]}')">
									</c:if>	
								</td>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function _remind(){
			...
			var form = document.getElementById('form1');
			form.action = '${pageContext.request.contextPath}/taxPay/remindCompanyList'; 
			form.method = "post";
			form.submit();
		}
	...
	function _export()
		{
		 ...
		 var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/taxPay/noCreateExport";
			form.method = "post";
			form.submit();
		}
		var timeoutId
