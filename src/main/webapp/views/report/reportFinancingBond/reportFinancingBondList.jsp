		...
		<!-- 	<span class="col-md-4">
		 ...
		 -->	
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_q,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_n,')==true }">	
					<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_dc,')==true }">
				...
				<%-- <c:if test="${fn:contains(gzwsession_code,',bimWork_cxlrzxzTb_n,')==true }">
					<input type="button" value="导出模板" class="form_btn" id="addbtn" onclick="_export(0)">
				</c:if> --%>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_cxhz,')==true }">
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td style="text-align: right;max-width:5em">
								...
								<td>
									...
									<c:if test="${sys.status==50114 || sys.status==50112  || sys.status==50115 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_e,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${sys.id}',${sys.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_r,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzTb_d,')==true && sys.status!=50115}">
