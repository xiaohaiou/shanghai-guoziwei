	...
	<body>
		...
		<!-- 	<input type = "hidden" id ="orgType" name="orgType">
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzCx_q,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_zqlrzxzCx_dc,')==true }">
					...
					<c:if test="${!empty dataList }">
						<c:forEach items="${dataList}" var="sys" varStatus="status">
							<tr>
								...
								<td style="text-align: center"><!-- right -->
									<fmt:formatNumber value="${sys[2]}"  pattern="#,##0.0000" />/<fmt:formatNumber value="${sys[3]}"  pattern="#,##0.0000" />
								</td>
							</tr>
						</c:forEach>
					</c:if>
