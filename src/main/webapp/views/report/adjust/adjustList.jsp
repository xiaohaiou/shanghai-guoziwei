	...
	<body>
		...
		<input type="hidden" name="groupType" value="${grouptype }">
			...
			<span class="col-md-12" style="text-align: right">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_new,')==true }">	
						<input type="button" value="数据导入" class="form_btn" id="addbtn" onclick="mload('${pageContext.request.contextPath}/reportcommon/newmodify?op=new&grouptype=${grouptype }&type=${type }')">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_exportfile,')==true }">
						<c:if test="${! empty templet.filePath }">
							<input type="button" class="form_btn" value="模板导出" onclick="mload('${pageContext.request.contextPath}/reportadjust/adjustTemplet')"></a>
						</c:if>
				</c:if>	
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status1">
							<tr>
								...
								<td>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_query,')==true }">	
									<a href="${pageContext.request.contextPath}/reportcommon/show?op=view&groupID=${sys.groupID}&organal=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }&type=${type }" checkdata="${sys.groupID},${sys.organalID },${sys.time}"  target="_blank" >查看</a>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_edit,')==true }">	
									<c:if test="${sys.examinestatus==50114 || sys.examinestatus==50112 }">
										<a href="javascript:void(0)"  onclick="mload('${pageContext.request.contextPath}/reportcommon/newmodify?op=modify&groupID=${sys.groupID}&organalID=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }&type=${type }')" >修改</a>
									</c:if>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_report,')==true }">	
									<c:if test="${sys.examinestatus!=50113 && sys.examinestatus!=50115 }">
										<a href="${pageContext.request.contextPath}/reportcommon/show?op=report&groupID=${sys.groupID}&organal=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }&type=${type }" checkdata="${sys.groupID},${sys.organalID },${sys.time}" target="_blank">上报</a>
									</c:if>
								</c:if>
								<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsb_delete,')==true }">
									<c:if test="${sys.examinestatus==50114 || sys.examinestatus==50112 }">
