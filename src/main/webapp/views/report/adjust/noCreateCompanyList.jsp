	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<span class="col-md-12" style="text-align: right">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hswcjgscx_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
						<input type="button" value="导出" class="form_btn" id="exportbtn" onclick="_export()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hswcjgscx_remind,')==true }">	<!-- bimWork_hswcjgscx_remind -->
					<input type="button" value="填报提醒" class="form_btn" id="qrybtn" onclick="_remind()">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hswcjgscx_remind_all,')==true }">	<!-- bimWork_hswcjgscx_remind_all -->
					<input type="button" value="全选" class="form_btn" id="qrybtn" onclick="_addRemindAll()">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hswcjgscx_disRemind_all,')==true }">	<!-- bimWork_hswcjgscx_disRemind_all -->
					<input type="button" value="不全选" class="form_btn" id="qrybtn" onclick="_removeRemindAll()">
				</c:if>					
			</span>
					...
					<c:if test="${not empty requestScope.msgPage.list}">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status1">
							<tr>
								...
								<td style="text-align: center">
									<c:if test="${fn:contains(gzwsession_code,',bimWork_hswcjgscx_remind_add_temp,')==true }">	<!-- bimWork_hswcjgscx_remind_add_temp -->
										<input type="button" value="+" class="form_btn" id="qrybtn" onclick="_addRemind('${sys[2]}','1')">
									</c:if>	
									<c:if test="${fn:contains(gzwsession_code,',bimWork_hswcjgscx_remind_remove_temp,')==true }">	<!-- bimWork_hswcjgscx_remind_remove_temp-->
										<input type="button" value="-" class="form_btn" id="qrybtn" onclick="_removeRemind('${sys[2]}')">
									</c:if>	
								</td>								
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
	   $(' input.time').jeDate(
		...
		function _remind(){
			...
			form.action = '${pageContext.request.contextPath}/reportadjust/remindCompanyList'; 
			form.method = "post";
			form.submit();
		}
         ...
         *  
         *  remindOrg 提醒公司编号 (必填*all)
         */
        function _addRemind(org,type){
        	$("input[name='remindOrg']").val(org);
         	$("input[name='remindKind']").val(type);       	
			var form = document.getElementById('form1');
			form.action = '${pageContext.request.contextPath}/reportadjust/noCreateCompanyList'; 
			form.method = "post";
			form.submit();
		}
		...
		function _addRemindAll(){
			_addRemind('all','3');
		}
	
		function _removeRemindAll(){
			_addRemind('all','4');
		}
		...
		{	
		 	var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/reportadjust/noCreateExport";
			form.method = "post";
			form.submit();
		}
		var timeoutId
