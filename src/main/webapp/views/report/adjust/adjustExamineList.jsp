	...
	<body>
		...
		<input type="hidden" name="groupType" value="${grouptype }">
			...
			<span class="col-md-12 " style="text-align: right">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsh_query,')==true }">	
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status1">
							<tr>
								...
								<td>
								<%-- 	onclick="checkdata('${sys.groupID}','${sys.organalID }','${sys.time}')" --%>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsh_query,')==true }">	
										<a href="${pageContext.request.contextPath}/reportcommon/show?op=view&type=${type }&groupID=${sys.groupID}&organal=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }"  target="_blank" checkdata="${sys.groupID},${sys.organalID },${sys.time}">查看</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsh_exam,')==true }">	
										<c:if test="${sys.examinestatus==50113}">
											<a href="${pageContext.request.contextPath}/reportcommon/show?op=examine&type=${type }&groupID=${sys.groupID}&organal=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }"  target="_blank" checkdata="${sys.groupID},${sys.organalID },${sys.time}" >审核</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_hsbbsh_return,')==true }">
									      <c:if test="${sys.examinestatus==50115}">
											  <a href="${pageContext.request.contextPath}/reportcommon/show?op=examine&type=${type }&groupID=${sys.groupID}&organal=${sys.organalID }&Date=${sys.time}&grouptype=${grouptype }"  target="_blank" checkdata="${sys.groupID},${sys.organalID },${sys.time}" >回退</a>
									       </c:if>
									</c:if>									
								</td>
							</tr>
