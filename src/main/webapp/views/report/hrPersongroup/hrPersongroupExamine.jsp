...
<body>
			...
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>审核财务人员资质数据
		...
		<form:form id="form2" modelAttribute="entityview" >
			<form:hidden path="id"/>
				...
				<h3 class="data_title1">财务人员花名册基本信息</h3>
				<div class="model_part">
				<label class="w20">时间:</label>
				<label class="w25 setleft">${ entityview.year}年${ entityview.month}月</label> 
				...
				<h3 class="data_title1">花名册列表</h3>
				
				<div class="model_part" id="nodeList">
				
				</div>
				<h3 class="data_title1">财务人员编制列表</h3>
				
				<div class="model_part" id="nodeLists">
				</div>
...
<script type="text/javascript">
		function initNodeList(v,pageNums){
			$.ajax({
				url:"${pageContext.request.contextPath}/hrPersongroup/"+v+"?pageNums="+pageNums+"&id=${entityview.id}",
				type:"POST",
				dataType:"text",
				async:false,
		...
		initNodeList("nodeList",1);//初始化节点列表
		initNodeList("nodeLists",1);//初始化节点列表
		function check()
		{
		...
		$("#commit-1").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/hrPersongroup/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/hrPersongroup/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
		    		var commitresult=JSON.parse(data);
							if(commitresult.result==true)
