		...
		<form:form id="form2" modelAttribute="entityview" >
			<form:hidden path="id"/>
				...
				<div class="label_inpt_div">
					...
					<h3 class="data_title1">应收债权(外部)信息(单位：万元)</h3>
...
<script type="text/javascript">
		function initNodeList(v,pageNums){
			$.ajax({
				url:"${pageContext.request.contextPath}/receivabledebtOuter/_itemViewList?pageNums="+pageNums+"&id=${entityview.id}",
				type:"POST",
				dataType:"text",
				async:false,
		...
		$("#commit-1").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/receivabledebtOuter/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50116},function(data){
		...
		$("#commit-2").click(function(){
			if(check())
			{
				$.post("/shanghai-gzw/receivabledebtOuter/commitexam",{entityid:$("#entityid").val(),examStr:$("#examStr").val(),examResult:50117},function(data){
		    		var commitresult=JSON.parse(data);
							if(commitresult.result==true)
