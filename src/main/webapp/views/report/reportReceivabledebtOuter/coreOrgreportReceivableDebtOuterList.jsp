	...
	<body>
		...
		<form id="form1"  class="row">
			...
			<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_hxqydeyszqcx_wb_query,')==true }">	
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_hxqydeyszqcx_wb_export,')==true }">
			...
			<div class="tablebox">
				<table>
					...
					<c:if test="${not empty requestScope.resultList }">
						<c:forEach items="${ requestScope.resultList}" var="sys" varStatus="status">
							<tr>
								<td>
									${status.index + 1 }
								</td>
								...
								<td>
									 <c:if test="${fn:contains(gzwsession_code,',bimWork_hxqydeyszqcx_wb_query,')==true }">	
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/detailinfoview?infoid=${sys.id}')" >查看</a>
									 </c:if>	
								</td>
							</tr>
						</c:forEach>
