	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_query,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_import,')==true }">	
					<input type="button" value="数据导入" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
			    </c:if>		
			    <c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_download,')==true }">	
					<input type="button" value="模板下载" class="form_btn" id="export" onclick="exportfile()">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_total,')==true }">	
					<input type="button" value="汇总查看" class="form_btn" id="exportbtn" onclick="mload('${mapurl}/collect')">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_export,')==true }">	
					<a href="${pageContext.request.contextPath}/views/report/taxTask/taxTask.xlsx" id="exportfile" style="display:none"></a>
				</c:if>
					...
					<c:if test="${!empty requestScope.msgPage.list }">
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.taxPlan}"  pattern="#,##0.00" />万元
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.favouredPolicy }"  pattern="#,##0.00" />万元
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.taxReturn }"  pattern="#,##0.00" />万元
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.inTaxReturn}"  pattern="#,##0.00" />万元
								</td>
								...
								<td>
									...
									<c:if test="${sys.status==50114 || sys.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_report,')==true }">	
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jsrwsjtb_delete,')==true }">
