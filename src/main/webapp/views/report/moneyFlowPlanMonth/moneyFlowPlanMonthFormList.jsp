	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">
					<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_q,')==true }">
						<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_n,')==true }">
						<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_hzcx,')==true }"> <!--bimWork_xjlyjhTb_hzcx-->
						<input type="button" value="汇总查询" class="form_btn" id="addbtn" onclick="mload('${mapurl}/g_query')">
					</c:if>
					<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_export,')==true }">
					...
					<c:if test="${!empty requestScope.msgPage.list}">
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_q,')==true }">
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/view?op=check&pid=${sys.pid}',${sys.pid },'')" >查看</a>
									</c:if>
									<c:if test="${sys.status==50114 || sys.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_e,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&pid=${sys.pid}',${sys.pid },${sys.status})" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_r,')==true }">
											<a href="javascript:void(0)" onclick="checkdata('${mapurl}/report?op=report&pid=${sys.pid}',${sys.pid },${sys.status})" >上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlyjhTb_d,')==true }">
											<a href="javascript:void(0)" onclick="del('${sys.pid}')" >删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
	...
	<script type="text/javascript">
		...
		function del(id){
			win.confirm('确定要删除？',function(r){
				if(r){
					var url = "/shanghai-gzw/moneyFlowPlanMonth/delete?pid=" + id;
		...
		function exam(id){
			win.confirm('确定要上报？',function(r){
				if(r){
					var url = "${mapurl}/saveandreport?pid=" + id;
					$.post(url, function(data) {
