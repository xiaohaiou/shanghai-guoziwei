	...
	<body>
		...
		<form id="form1"  class="row"  >
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_query,')==true }">	
					<input type="button" value="查询" class="form_btn" id="qrybtn" onclick="_query()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_new,')==true }">	
					<input type="button" value="新增" class="form_btn" id="addbtn" onclick="mload('${mapurl}/newmodify')">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_hzcx,')==true }"> <!--bimWork_xjlzzxsjtb_hzcx-->
					<input type="button" value="汇总查询" class="form_btn" id="addbtn" onclick="mload('${mapurl}/g_query')">
				</c:if>	
				<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_export,')==true }">
					  ...
					  <c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="sys" varStatus="status">
							<tr>
								...
								<td>
									${sys.week }周(${sys.weekStartDate}~${sys.weekEndDate})
								</td>
								<td style="text-align: left;max-width: 12em">
									${sys.orgname }
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.cashInto }"  pattern="#,##0.00" />万元
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.cashOut }"  pattern="#,##0.00" />万元
								</td>
								<td style="text-align: right">
									<fmt:formatNumber value="${sys.cashNetInflow }"  pattern="#,##0.00" />万元
								</td>
								...
								<td>
									...
									<c:if test="${sys.status==50114 || sys.status==50112 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_edit,')==true }">	
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/newmodify?op=modify&id=${sys.id}',${sys.id })" >修改</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_report,')==true }">	
										<a href="javascript:void(0)" onclick="checkdata('${mapurl}/postexam?op=report&id=${sys.id}',${sys.id })">上报</a>
										</c:if>
										<c:if test="${fn:contains(gzwsession_code,',bimWork_xjlzzxsjtb_delete,')==true }">
