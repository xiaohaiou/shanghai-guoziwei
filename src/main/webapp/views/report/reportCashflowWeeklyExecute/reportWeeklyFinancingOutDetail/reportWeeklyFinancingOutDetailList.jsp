...
<div class="tablebox" style="overflow-x: auto">
	<table style="width: 250em">
		<tr class="table_header" id="reportWeeklyFinancingOutDetailTableTr">
			<th>序号</th>
			<!-- <th>成员公司</th>
			<th>所属核心企业</th> -->
			<th>承贷主体</th>
		...
		<c:if test="${not empty requestScope.msgPage.list }">
			<c:forEach items="${requestScope.msgPage.list }" var="node" varStatus="status">
				<tr>
					...
					<td>
						<input type="text" id="loanCompName" value="${node.loanCompName }" check="NotEmpty_string_._._._._." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail',this)">
					...
					<td>
						<input type="text" id="repayBank" value="${node.repayBank }" check="NotEmpty_string_._._._._." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail',this)">
					</td>
					<td>
						<select id="financingType" onchange="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail',this)">
					...
					<td>
						<select id="financingTypeDetail" onchange="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail',this)">
					...
					<td>
						<input type="text" id="repayAmount" value="${node.repayAmount }" check="NotEmpty_double_16_2_0_。_." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail',this)">
					</td>
					<td>
						<select id="currency" onchange="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail',this)">
					...
					<td>
						<a class="btn btn-primary model_btn_ok" onclick="del('${(msgPage.pageNum-1)*5+status.count }','reportWeeklyFinancingOutDetail')">删除</a>
					</td>
				</tr>
			</c:forEach>
		</c:if>
...
<script type="text/javascript">
	...
	function changeDate3(val,text){
		 ...
		 obj.id="repayDate";
		 obj.value=val;
		 modifyEntity(text,'reportWeeklyFinancingOutDetail',obj);
	}
	...
	$("#out_com_ztree_1").click(function(){
		...
		setTimeout(function(){
  				if($.fn.zTree.getZTreeObj("out_com_ztree_1").getSelectedNodes()[0])
				{
					 ...
					 modifyComp(text,'reportWeeklyFinancingOutDetail',obj);
				}
	   	});
