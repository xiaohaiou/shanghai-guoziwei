...
<body>
	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<span class="col-md-4">是否涉及整改：
				<select name="isChange" class="selectpicker" style="width: calc(100% - 9em);">
					<option value="">全部</option>
					<option value="1" <c:if test="${entity.isChange == 1}">selected="selected"</c:if>>是</option>
					<option value="0" <c:if test="${entity.isChange == 0}">selected="selected"</c:if>>否</option>
				</select> 
			</span>
			<br>
			<br>
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_ndjhgsfxwtqd_query,')==true }">
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="item" varStatus="status">
						<tr>
							...
							<td style="text-align: left;max-width:20em;">${item.compName}</td>
							<td style="text-align: left;max-width:20em;">${item.itemName}</td>
							<td style="text-align: left;max-width:20em;">${item.problem}</td>
							<td style="text-align: center;">${ item.isChange==0? "否" :"是"}</td>
							<td style="text-align: left;max-width:20em;">${item.changeContent}</td>
							<td>
							<c:if test="${fn:contains(gzwsession_code,',bimWork_ndjhgsfxwtqd_view,')==true }">
