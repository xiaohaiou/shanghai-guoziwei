	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<div class="form_div col-md-12">
				<c:if
					test="${fn:contains(gzwsession_code,',bimWork_jhxmsxxxwh_query,')==true }">
					<input type="button" value="查询" class="form_btn" id="qrybtn"
						onclick="_query()">
				</c:if>
				<c:if
					test="${fn:contains(gzwsession_code,',bimWork_jhxmsxxxwh_export,')==true }">
				...
				<c:if test="${!empty requestScope.msgPage.list }">
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="l"
						...
						<tr>
							...
							<td>${l.statusName }</td>
							<td><c:if
									test="${fn:contains(gzwsession_code,',bimWork_jhxmsxxxwh_view,')==true }">
									<a href="javascript:void(0)"
										onclick="mload('${mapurl}/view?type=${type }&id=${l.id }')">查看</a>
								</c:if> 
									<c:if
										test="${fn:contains(gzwsession_code,',bimWork_jhxmsxxxwh_exam,')==true }">
										<a href="javascript:void(0)" onclick="mload('${mapurl}/manageedit?id=${l.id }')">修改</a>
									</c:if>
									
									<c:if test="${l.status==80040 }">
										<c:if test="${fn:contains(gzwsession_code,',bimWork_jhxmsxxxwh_sqbj,')==true }">
											
												<a href="javascript::void(0);" onclick="changeStatus('${l.id}')">申请办结</a>
...
<script type="text/javascript">
		...
		function _export(){
			var form = document.forms[0];
			form.action = "${pageContext.request.contextPath}/inspectproject/moneyExport";
			form.method = "post";
			form.submit();
		}
		...
		function changeStatus(id){
			parent.win.confirm('确定要申请办结？',function(r){
				if(r){
					var url = "${pageContext.request.contextPath}/inspectproject/updatestatus?id=" + id ;
					$.post(url, function(data) {
						var commitresult1=JSON.parse(data);
						if(commitresult1.result){
							win.successAlert('申请办结成功！',_query);
						}else{
							win.errorAlert('申请办结失败！',_query);
						}
