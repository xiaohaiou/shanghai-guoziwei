	...
	<div class="table_content">
		<form id="form1" class="row">
			...
			<span class="col-md-4"> <span class="search_title">稽核公司：</span>
				...
				<input name="compName" id="compName" value="" readonly title="${entity.compName}">
			...
			<div class="form_div col-md-12">
			    <c:if test="${fn:contains(gzwsession_code,',bimWork_ldshgsjhsx_query,')==true }">
		...
		<div class="tablebox">
			<table>
				<tr class="table_header">
					...
					<th style="width:10%">稽核类型</th>
					<th style="width:20%" >稽核公司名称</th>
					<th style="width:20%" >所属核心企业</th>
					<th style="width:10%" >发现问题总数</th> 
					<th style="width:10%" >整改问题数</th>
					...
					<c:forEach items="${ requestScope.msgPage.list}" var="item" varStatus="status">
						<tr>
							...
							<td style="text-align: left;">
							  <c:if test="${fn:contains(gzwsession_code,',bimWork_ldshgsjhsx_view,')==true }">
							  	<a href="javascript:void(0)" onclick="mload(encodeURI('${mapurl}/leaderCheckView?year=${item[0]}&inspectType=${item[1]}&compId=${item[3]}&compName=${item[4]}'))">查看</a>
							  </c:if>
							  <c:if test="${fn:contains(gzwsession_code,',bimWork_ldshgsjhsx_check,')==true }">
								  <c:if test="${item[9] == 1}">
								  	<a href="javascript:void(0)" onclick="mload(encodeURI('${mapurl}/leaderCheckView?year=${item[0]}&inspectType=${item[1]}&compId=${item[3]}&compName=${item[4]}&isEdit=true'))">审核</a>
								  </c:if>
							  </c:if>
							</td>
						</tr>
