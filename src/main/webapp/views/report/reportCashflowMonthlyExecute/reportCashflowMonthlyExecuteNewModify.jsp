...
<body>
	<c:choose>
		<c:when test="${op eq 'modify'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>修改现金流月执行数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>新增现金流月执行数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	...
	<div class="label_inpt_div">
		<form:form id="form2" modelAttribute="entityview" >
			<form:hidden path="id"/>
			<input type="hidden" id="oldCompId" value="${entityview.org}">
			...
			<div class="model_part">
				...
				<label class="w20"><span class="required"> * </span>归还本金金额:</label>
				<form:input class="w25" path="returnCapitalAmounts" id="returnCapitalAmounts" check="NotEmpty_double_16_2_0+_。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>归还利息金额:</label>
				<form:input class="w25" path="returnInterestAmounts" id="returnInterestAmounts" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>归还票据金额:</label>
				<form:input class="w25" path="returnNoteAmounts" id="returnNoteAmounts" check="NotEmpty_double_16_2_0+_。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>归还租金金额:</label>
				<form:input class="w25" path="returnRentAmounts" id="returnRentAmounts" check="NotEmpty_double_16_2_0+_。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>筹资性流出:</label>
				<form:input class="w25" path="financingOut" id="financingOut" readonly="true"/>
			...
			<div class="model_part">
				...
				<form:input class="w25" path="investmentInto" id="investmentInto" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>在建工程:</label>
				<form:input class="w25" path="constructionInProcess" id="constructionInProcess" check="NotEmpty_double_16_2_._._." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>飞机投资:</label>
				<form:input class="w25" path="airplaneInvest" id="airplaneInvest" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>股权投资:</label>
				<form:input class="w25" path="stockEquityInvest" id="stockEquityInvest" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>土地储备:</label>
				<form:input class="w25" path="landReserve" id="landReserve" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>固定资产购置:</label>
				<form:input class="w25" path="fixedAssetsPurchase" id="fixedAssetsPurchase" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>金融性投资:</label>
				<form:input class="w25" path="financeInvest" id="financeInvest" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
				<label class="w20"><span class="required"> * </span>其它投资性支出:</label>
				<form:input class="w25" path="elseInvestExpense" id="elseInvestExpense" check="NotEmpty_double_16_2_._。_." onblur="calrate()"/>
			...
			<div>
			     <input type="button" value="导入" class="form_btn" id="qrybtn" onclick="importdata()">
			</div>
			<h3 class="data_title1">筹资性流入明细列表
				...
				<a class="model_btn_plus" id="reportMonthlyFinancingIntoDetail" onclick="addEntity(this)">+</a>
			</h3>
			<div class="model_part">
			...
			<h3 class="data_title1">筹资性流出明细列表
				...
				<a class="model_btn_plus" id="reportMonthlyFinancingOutDetail" onclick="addEntity(this)">+</a>
			</h3>
			<div class="model_part">
			...
			<h3 class="data_title1">投资性流出明细列表
				...
				<a class="model_btn_plus" id="reportMonthlyInvestmentOutDetail" onclick="addEntity(this)">+</a>
			</h3>
			<div class="model_part">
...
<script type="text/javascript">
		...
		function exportExcel(name,dataNum){
			...
			excelName.value = $("#checkedCompanyName").val()+$("#year").val()+"年"+$("#month").val()+"月"+name;
			...
			if(dataNum == '1'){
				...
				v="reportMonthlyFinancingIntoDetail";
			}else if(dataNum == '2'){
				if(data2.length != 0){
					excelData.value = JSON.stringify(data2);
				}
				v="reportMonthlyFinancingOutDetail";
			}else{
				if(data3.length != 0){
					excelData.value = JSON.stringify(data3);
				}
				v="reportMonthlyInvestmentOutDetail";
			}
...
/* 		//获取此公司此年份此月份的周执行中各明细表的数据
		function getWeeksData(year){
			if(year == ""){
				year = $("#year").val();
			}
			var compId = $("#organalID").val();
			var month = $("#month").val();
			var oldCompId = $("#oldCompId").val();
			if(oldCompId == ""){
				if(compId != "" && year != "" && month != ""){
					$.ajax({
						type:"POST",
						url:"${pageContext.request.contextPath}/reportCashflowMonthlyExecute/getWeeksData",
						dataType:'json',
						data:{
							compId:compId,
							year:year,
		...
		function addEntity(obj){
			...
			var v=obj.id;
			var newEntity= {};
			if(v == 'reportMonthlyFinancingIntoDetail'){
				newEntity.id=null;
				newEntity.monthId="";
			...
			if(v == 'reportMonthlyFinancingOutDetail'){
				newEntity.id=null;
				newEntity.monthId="";
				newEntity.memberCompId=$("#organalID").val();
				newEntity.memberCompName=$("#checkedCompanyName").val();
			...
			if(v == 'reportMonthlyInvestmentOutDetail'){
				newEntity.id=null;
				newEntity.monthId="";
				newEntity.memberCompId=$("#organalID").val();
				newEntity.memberCompName=$("#checkedCompanyName").val();
		...
		$("#commit-1").click(function(){
			...
			var url = "${pageContext.request.contextPath}/reportCashflowMonthlyExecute/save";
		...
		$("#commit-2").click(function(){
			...
			if(!vaild.all() || !commitcheck())
			{
					...
					//var entityBasicInfo = $('#form2').serialize();
			var url = "${pageContext.request.contextPath}/reportCashflowMonthlyExecute/saveandreport";
		...
		$(' input.time').jeDate(
			{
				...
				clearfun:function(elem, val) {changeDate2('');},            //清除日期后的回调, elem当前输入框ID, val当前选择的值
	  				okfun:function(elem, val, date) {changeDate2(val);}, 
			}
		)
	   				...
	   				if($.fn.zTree.getZTreeObj("com_ztree").getSelectedNodes()[0])
						{
							 ...
							 var url = "${pageContext.request.contextPath}/reportCashflowMonthlyExecute/getCoreComp";
							 $.ajax({
							 	type:'POST',
							 	...
							 	success:function(data){
							 		...
							 		$("#cashOut").val('');
							 		$("#cashNetInflow").val('');
							 		$("#returnCapitalAmounts").val('');
							 		$("#returnInterestAmounts").val('');							 		
							 		$("#returnNoteAmounts").val('');							 		
							 		$("#returnRentAmounts").val('');							 			
							 	}
							 });
							 var year=$("#year").val();
							 getWeeksData(year);
		...
		function calrate()
		{
			...
			//归还本金金额
			var returnCapitalAmounts=$("#returnCapitalAmounts").val();
			//归还利息金额
			var returnInterestAmounts=$("#returnInterestAmounts").val();
			//归还票据金额
			var returnNoteAmounts=$("#returnNoteAmounts").val();
			//归还租金金额
			var returnRentAmounts=$("#returnRentAmounts").val();
			//筹资性流出
			var financingOut="";
			//筹资性净流量
			...
			//计算筹资性流出、净流量
			if(returnCapitalAmounts != "" && returnInterestAmounts != "" && returnNoteAmounts != "" && returnRentAmounts != "" ){
				financingOut=(parseFloat(returnCapitalAmounts)+parseFloat(returnInterestAmounts)+parseFloat(returnNoteAmounts)+parseFloat(returnRentAmounts)).toFixed(2);
				$("#financingOut").text(financingOut);
				$("#financingOut").val(financingOut);
			}else{
				$("#financingOut").text("");
				$("#financingOut").val("");
				financingOut="";
		...
		function sumData(){
		 ...
		 var url = "${pageContext.request.contextPath}/reportCashflowMonthlyExecute/sumDataForNew";
		 $.ajax({
		 	type:'POST',
		 	data:{org:org,year:year,month:month},
		 	dataType:'json',
		 	url:url,
		 	success:function(data){
				...
				$("#returnCapitalAmounts").val(data.returnCapitalAmounts);
		 		$("#returnInterestAmounts").val(data.returnInterestAmounts);							 		
		 		$("#returnNoteAmounts").val(data.returnNoteAmounts);							 		
		 		$("#returnRentAmounts").val(data.returnRentAmounts);

		 		$.each(data.list1, function(i,item){				 		
					data1.push(item);
			    });
				initNodeList('reportMonthlyFinancingIntoDetail',1,"add",data1);//初始化筹资性流入明细列表
		 			 		
		 		$.each(data.list2, function(i,item){				 					
					data2.push(item);
			    });
				initNodeList('reportMonthlyFinancingOutDetail',1,"add",data2);

				$.each(data.list3, function(i,item){				 		
					data3.push(item);
			    });
				initNodeList('reportMonthlyInvestmentOutDetail',1,"add",data3);		 						 						 		
			}
		 }); 
		}
