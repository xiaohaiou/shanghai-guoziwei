...
<div class="tablebox" style="overflow-x: auto">
	...
	<table style="width: 250em">
		<tr class="table_header" id="reportMonthlyFinancingIntoDetailTableTr">
			<th>序号</th>
			<!-- <th>成员公司</th>
			<th>所属核心企业</th> -->
			<th>承贷主体</th>
		...
		<c:if test="${not empty requestScope.msgPage.list }">
			<c:forEach items="${requestScope.msgPage.list }" var="node" varStatus="status">
				<tr>
					...
					<td>
						<input type="text" id="loanCompName" value="${node.loanCompName }" check="NotEmpty_string_._._._._." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					...
					<td>
						<input type="text" id="loanBank" value="${node.loanBank }" check="NotEmpty_string_._._._._." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					</td>
					<td>
						<input type="text" id="loanAmount" value="${node.loanAmount }" check="NotEmpty_double_16_2_0+_。_." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					</td>
					<td>
						<input class="w25 date" type="text" id="accountDate" readonly="true" check="NotEmpty_string_._._._._." value="${node.accountDate }" />
					</td>
					<td>
						<input class="w25 date1" type="text" id="dueDate" readonly="true" check="NotEmpty_string_._._._._." value="${node.dueDate }" />
					</td>
					<td>
						<input type="text" id="lengthOfMaturity" value="${node.lengthOfMaturity }" check="NotEmpty_string_._._._._." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					</td>
					<td>
						<input type="text" id="financingAccountCost" value="${node.financingAccountCost }" check="NotEmpty_double_16_2_0+_。_." onblur="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					</td>
					<td>
						<select id="type" onchange="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					...
					<td>
						<select id="loanType" onchange="modifyEntity('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail',this)">
					...
					<td>
						<a class="btn btn-primary model_btn_ok" onclick="del('${(msgPage.pageNum-1)*5+status.count }','reportMonthlyFinancingIntoDetail')">删除</a>
					</td>
				</tr>
			</c:forEach>
		</c:if>
...
<script type="text/javascript">
	...
	function changeDate(val,text){
		 ...
		 obj.id="accountDate";
		 obj.value=val;
		 modifyEntity(text,'reportMonthlyFinancingIntoDetail',obj);
	}
	...
	$("#detail_com_ztree_1").click(function(){
		...
		setTimeout(function(){
  				if($.fn.zTree.getZTreeObj("detail_com_ztree_1").getSelectedNodes()[0])
				{
					 ...
					 modifyComp(text,'reportMonthlyFinancingIntoDetail',obj);
				}
	   	});
