...
<!--查看按钮界面  -->
	<c:choose>
		<c:when test="${op eq 'report'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>上报管理口径核算数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>查看管理口径核算数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
		...
		<form:form id="form2" modelAttribute="entityview" >
				...
				<label class="w25 setleft"><fmt:formatNumber value="${ entityview.ebitda}"  pattern="#,##0.000000" /></label>  
				<label class="w20"><span class=""> * </span>实收资本(亿元):</label>
			...
			<!-- 	<label class="w20"></label>
				...
				<label class="w25 setleft"><fmt:formatNumber value="${ entityview.ebitdaMargins}"  pattern="#,##0.00" /></label> 
													<!-- 此处字段需要修改 -->
				<label class="w20"><span class="required"> * </span>EBITDA利息保障倍数:</label>
				<label class="w25 setleft"><fmt:formatNumber value="${ entityview.interestCover}"  pattern="#,##0.00" /></label> 
													<!-- 此处字段需要修改 -->
				<label class="w20"><span class="required"> * </span>偿债备付率(%):</label>
				<label class="w25 setleft"><fmt:formatNumber value="${ entityview.repaymentRate}"  pattern="#,##0.00" /></label> 
													<!-- 此处字段需要修改 -->
...
<script type="text/javascript">
		...
		function exam(id){
			...
			var url = "/shanghai-gzw/reportManageAdjust/saveandreport?id=" + id;
