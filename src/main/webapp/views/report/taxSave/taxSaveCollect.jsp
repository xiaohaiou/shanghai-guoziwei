...
<body>
	<h4>
		<span class="glyphicon glyphicon-pencil"></span>节税金额数据汇总查看
		...
		<form:form id="form2" modelAttribute="entityview" >	
			<div class="label_inpt_div">
				<div class="model_part">
				       <span class="required"><font color=red>*</font></span>使用时间：
							<input  type="text" name="starttime" value="${entityview.starttime }" readonly="true" class="time time_short" check="NotEmpty"/>
							至
							<input  type="text" name="endtime" value="${entityview.endtime }" readonly="true" class="time time_short" check="NotEmpty"/>
					    </span>
				...
				<span class="w25">
					...
					<input id="company" name="orgName" style="width: 100% !important;" id="company" value="${entityview.orgName }" check="NotEmpty_string_._._._._." readonly>
				   ...
				   <div class="model_btn">
				        ...
				        <input name="inTaxReturn" type="hidden"  value="${entityview.inTaxReturn}"  />
				        <input name="taxReturn" type="hidden"  value="${entityview.taxReturn}"  />
				        <input name="taxPlan" type="hidden"  value="${entityview.taxPlan}"  />
				        <input name="favouredPolicy" type="hidden"  value="${entityview.favouredPolicy}"  />
				...
				<div>	
					<h3 class="data_title1">汇总数据</h3>
						<div class="model_part">
							<label class="w20"><font color=red>*</font>本月节税汇总金额（万元）:</label>
					        <input name="sumTaxSave" readonly="readonly"  id ="sumTaxSave" class="w25"  value="${entity.taxSave}" /> 
					        <label class="w20"><font color=red>*</font>非税财政返还本期汇总节税(万元):</label>
					        <input name="sumTaxSave"  readonly="readonly" id ="sumTaxSave" class="w25"  value="${entity.inTaxReturn}" /> 
					        <label class="w20"><font color=red>*</font>税财政返还本期汇总节税(万元):</label>
					        <input name="sumTaxSave" readonly="readonly" id ="sumTaxSave" class="w25"  value="${entity.taxReturn}" /> 
					        <label class="w20"><font color=red>*</font>纳税筹划本期汇总节税(万元):</label>
					        <input name="sumTaxSave" readonly="readonly" id ="sumTaxSave" class="w25"  value="${entity.taxPlan}" /> 
					        <label class="w20"><font color=red>*</font>优惠政策及减免税申请本期汇总节税(万元):</label>
					        <input name="sumTaxSave" readonly="readonly" id ="sumTaxSave" class="w25"  value="${entity.favouredPolicy}" /> 
							 <div class="tablebox">
									      <table>
									            <tr class="table_header">
									                 <th>单位</th>
									                 <th>本月节税汇总金额（万元）</th>
									                 <th>非税财政返还本期汇总节税(万元)</th>
									                 <th>税财政返还本期汇总节税(万元)</th>
									                 <th>纳税筹划本期汇总节税(万元)</th>
									                 <th>惠政策及减免税申请本期汇总节税(万元)</th>
									            </tr>
...
<script type="text/javascript">
		...
		*/
		...
		$("#commit_1").click(function(){
			...
			var url="${pageContext.request.contextPath}/taxSave/saveOrUpdate?op=save";
		...
		$("#commit_2").click(function(){
			...
			if(!vaild.all() || !checkcommit())
			{
					...
					//var entityBasicInfo = $('#form2').serialize();
			var url = "${pageContext.request.contextPath}/taxSave/saveAndReport?op=saveR";
