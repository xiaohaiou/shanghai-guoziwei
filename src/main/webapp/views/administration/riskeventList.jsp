	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<div class="form_div col-md-12">
				...
				<c:if test="${fn:contains(gzwsession_code,',bimWork_bmfxsjsjsb_new,')==true }">
					<input type="button" value="新增" class="form_btn" id="model_add" onclick="mload('${pageContext.request.contextPath}/adRiskevent/addOrModify?op=add')">
				</c:if>
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="riskevent" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${fn:contains(gzwsession_code,',bimWork_bmfxsjsjsb_edit,')==true }">
										<c:if test="${riskevent.status==50114 || riskevent.status==50112 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adRiskevent/addOrModify?op=modify&id=${riskevent.id}&lastModifyDate=${riskevent.lastModifyDate }','${riskevent.id }')" >修改</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_bmfxsjsjsb_report,')==true }">
										<c:if test="${riskevent.status==50114 || riskevent.status==50112 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adRiskevent/view?op=reported&id=${riskevent.id}&lastModifyDate=${riskevent.lastModifyDate }','${riskevent.id }')">上报</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_bmfxsjsjsb_delete,')==true }">
										<c:if test="${riskevent.status==50114 || riskevent.status==50112 }">
											<a href="javascript:;" onclick="del('${riskevent.id}','${riskevent.lastModifyDate }')">删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
				...
				if(r){
					$.post("${pageContext.request.contextPath}/adRiskevent/checkDelete?id=" + id, function(data) {
						...
						if(commitresult.result){
							var url = "${pageContext.request.contextPath}/adRiskevent/delete?id=" + id +"&lastModifyDate=" + lastModifyDate;
