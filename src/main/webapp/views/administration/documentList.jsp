		...
		<form:form id="form1" class="row"  >
		   ...
		   <span class="col-md-4">
			...
			<div class="form_div col-md-12">
				...
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsb_new,')==true }">	
					<input type="button" value="新增" class="form_btn" id="model_add" onclick="mload('${pageContext.request.contextPath}/adDocument/addOrModify?op=add')">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsb_tb,')==true }">	
				<input type="button" value="同步" class="form_btn" onclick="tb()">
				</c:if>
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsb_export,')==true }">	<!-- bimWork_gwsjsb_export -->
					<input type="button" value="导出" class="form_btn" onclick="_export()">
				</c:if>
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="document" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsb_edit,')==true }">
										<c:if test="${document.status==50114 || document.status==50112 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adDocument/addOrModify?op=modify&id=${document.id}&lastModifyDate=${document.lastModifyDate }','${document.id }')" >修改</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsb_report,')==true }">
										<c:if test="${document.status==50114 || document.status==50112 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adDocument/view?op=reported&id=${document.id}&lastModifyDate=${document.lastModifyDate }','${document.id }')">上报</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsb_delete,')==true }">
										<c:if test="${document.status==50114 || document.status==50112 }">
											<a href="javascript:;" onclick="del('${document.id}','${document.lastModifyDate }')">删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
