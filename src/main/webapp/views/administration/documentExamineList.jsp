		...
		<form:form id="form1" class="row"  >
		   ...
		   <span class="col-md-4">
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsh_query,')==true }">
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="document" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsh_query,')==true }">
										<a href="javascript:;" onclick="checkDelete('${pageContext.request.contextPath}/adDocument/view?op=check&id=${document.id}','${document.id }')">查看</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsh_exam,')==true }">
										<c:if test="${document.status==50113 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adDocument/examine?id=${document.id}&lastModifyDate=${document.lastModifyDate }','${document.id }')">审核</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_gwsjsh_examback,')==true }"> <!-- bimWork_gwsjsh_examback -->
										<c:if test="${document.status==50115 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adDocument/examine?id=${document.id}&lastModifyDate=${document.lastModifyDate }','${document.id }')">退回</a>
										</c:if>
									</c:if>									
								</td>
							</tr>
