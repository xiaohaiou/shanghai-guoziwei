...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<span class="col-md-4">
				<span class="search_title">类别：</span>
		   ...
		   <span class="col-md-4">
			...
			<div class="form_div col-md-12">
				...
				<c:if test="${fn:contains(gzwsession_code,',bimWork_yqsjsjsb_new,')==true }">
					<input type="button" value="新增" class="form_btn" id="model_add" onclick="mload('${pageContext.request.contextPath}/adImportant/addOrModify?op=add')">
				</c:if>
						...
						<c:forEach items="${ requestScope.msgPage.list}" var="important" varStatus="status">
							<tr>
								...
								<td>
									...
									<c:if test="${fn:contains(gzwsession_code,',bimWork_yqsjsjsb_edit,')==true }">
										<c:if test="${important.status==50114 || important.status==50112 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adImportant/addOrModify?op=modify&id=${important.id}&lastModifyDate=${important.lastModifyDate }','${important.id }')" >修改</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_yqsjsjsb_report,')==true }">
										<c:if test="${important.status==50114 || important.status==50112 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adImportant/view?op=reported&id=${important.id}&lastModifyDate=${important.lastModifyDate }','${important.id }')">上报</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_yqsjsjsb_delete,')==true }">
										<c:if test="${important.status==50114 || important.status==50112 }">
											<a href="javascript:;" onclick="del('${important.id}','${important.lastModifyDate }')">删除</a>
										</c:if>
									</c:if>
								</td>
							</tr>
				...
				if(r){
					$.post("${pageContext.request.contextPath}/adImportant/checkDelete?id=" + id, function(data) {
						...
						if(commitresult.result){
							var url = "${pageContext.request.contextPath}/adImportant/delete?id=" + id +"&lastModifyDate=" + lastModifyDate;
