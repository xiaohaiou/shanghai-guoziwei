...
<html>
	...
	<body>
		...
		<form:form id="form1" class="row"  >
			...
			<span class="col-md-4">
				<span class="search_title">数据名称：</span>
			...
			<div class="form_div col-md-12">
				<c:if test="${fn:contains(gzwsession_code,',bimWork_dbsjsh_query,')==true }">
					<input type="button" value="查询" class="form_btn" id="qrybtn">
				</c:if>
					<input type="button" value="新增" class="form_btn" id="qrybtn">
			</div>
		</form:form>
			<div class="tablebox">
				<table>
					<tr class="table_header">
						...
						<th width="5%">年份</th>
						<th width="10%">单位名称</th>
						<th width="10%">预算信息名称</th>
						<th width="10%">预算信息总量</th>
						<th width="8%">预算信息类型</th>
						<th width="8%">预算信息增长率</th>
						<th width="5%">创建人</th>
						<th width="5%">审核人</th>
					...
					<c:set var="recordNumber" value="${(msgPage.pageNum - 1) * 10 }" />
						<c:forEach items="${ requestScope.msgPage.list}" var="supervise" varStatus="status">
							<tr>
								...
								<td style="text-align: center;">
									${supervise.totalDivision }
								</td>
								<td style="text-align: center;">
									${supervise.propelDivision }
								</td>
								<td style="text-align: center;">
									${supervise.finishDivision }
								</td>
								<td style="text-align: center;">
								...
								<td>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_dbsjsh_query,')==true }">
										<a href="javascript:;" onclick="checkDelete('${pageContext.request.contextPath}/adSupervise/view?op=check&id=${supervise.id}','${supervise.id }')">查看</a>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_dbsjsh_exam,')==true }">
										<c:if test="${supervise.status==50113 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adSupervise/examine?id=${supervise.id}&lastModifyDate=${supervise.lastModifyDate }','${supervise.id }')">审核</a>
										</c:if>
									</c:if>
									<c:if test="${fn:contains(gzwsession_code,',bimWork_dbsjsh_examback,')==true }"> <!-- bimWork_dbsjsh_examback -->
										<c:if test="${supervise.status==50115 }">
											<a href="javascript:void(0)" onclick="checkDelete('${pageContext.request.contextPath}/adSupervise/examine?id=${supervise.id}&lastModifyDate=${supervise.lastModifyDate }','${supervise.id}')">退回</a>
										</c:if>
									</c:if>										
								</td>
							</tr>
