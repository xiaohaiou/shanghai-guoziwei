...
<body>
	<c:choose>
		<c:when test="${op eq 'check'}">
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>查看监管预警数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:when>
		<c:otherwise>
			<h4>
				<span class="glyphicon glyphicon-pencil"></span>上报监管预警数据
				<i class="iconfont icon-guanbi"></i>
			</h4>
		</c:otherwise>
	</c:choose>
	...
	<form:form id="form2" modelAttribute="theEntity" method="post">
		...
		<div class="label_inpt_div">
			<div class="model_part">
				...
				<label class="w20">监管预警数据单位名称:</label>
				<label class="w25 setleft">${ theEntity.riskCompName}</label>
				<label class="w20">监管预警数据所属核心企业:</label>
				...
				<label class="w20">监管预警数据标题:</label>
				<label class="w25 setleft">${ theEntity.eventTitle}</label>
				<label class="w20" style="vertical-align:top;">监管预警数据概要:</label>
...
<script type="text/javascript">
		...
		function exam(id){
			...
			var url = "/shanghai-gzw/adRiskevent/reported?id=" + id +"&lastModifyDate=" + lastModifyDate;
