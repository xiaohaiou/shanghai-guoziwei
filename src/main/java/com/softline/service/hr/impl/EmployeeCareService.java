...
@Service("employeeCareService")
/**
 * 
 * @author zy
 *
 */
public class EmployeeCareService implements IEmployeeCareService {
	...
	@Autowired
	@Qualifier("employeeCareDao")
	private IEmployeeCareDao employeeCareDao;
	 ...
	 * @return
	 */
	public MsgPage getSocialVoluntaryListView(HhEmployeeCare entity,
			Integer curPageNum, Integer pageSize, Integer fun) {
		MsgPage msgPage = new MsgPage();
		// 总记录数
		Integer totalRecords = employeeCareDao.getSocialVoluntaryListCount(
				entity, fun);
		// 当前页开始记录
		int offset = msgPage.countOffset(curPageNum, pageSize);
		// 分页查询结果集
		List<HhEmployeeCare> list = employeeCareDao
				.getSocialVoluntaryList(entity, offset, pageSize, fun);
		// 数据格式转换
		List<DataSocialVoluntaryView> View = new ArrayList<DataSocialVoluntaryView>();
		for (HhEmployeeCare voluntary : list) {
	...
	public HhEmployeeCare getSocialVoluntary(Integer id) {
		return employeeCareDao.getSocialVoluntary(id);
	}
	 ...
	 * @return
	 */
	public CommitResult saveSocialVoluntary(HhEmployeeCare entity,
		...
		if (entity.getStatus().equals(Base.examstatus_2)) {
			...
			portalMsg.setTitle(entity.getYear()+"年"+entity.getMonth()+"月员工关爱数据未审核");
			...
			portalMsg.setUrl("/shanghai-gzw/hr/employeecare/examinelist?id="+entity.getId());
	...
	public CommitResult deleteSocialVoluntary(Integer id, HhUsers use) {
		HhEmployeeCare entity = employeeCareDao.getSocialVoluntary(id);
		CommitResult result = new CommitResult();
		if (entity != null) {
	...
	public CommitResult saveSocialVoluntaryExamine(Integer entityID,
		...
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		HhEmployeeCare entity = getSocialVoluntary(entityID);
		// 审核不通过
		...
		HhEmployeeCare voluntary = (HhEmployeeCare) voluntaryResult
				.getEntity();
		Integer examineentityid = voluntary.getId();
		// 保存审核信息
		examineService.saveExamine(examineentityid, Base.examkind_employeecare,
	 ...
	 * @return
	 */
	private CommitResult saveCheck(HhEmployeeCare entity, String type) {
		CommitResult result = new CommitResult();
		if (!employeeCareDao.saveSocialVoluntaryRepeatCheck(entity, type)) {
		...
		if (!employeeCareDao.checkCanEdit(entity)) {
			result = CommitResult.createErrorResult("该数据已经被上报,不能修改!");
			return result;
		}
		result.setResult(true);
	...
	private CommitResult examineCheck(Integer entityID) {
		CommitResult result = new CommitResult();
		HhEmployeeCare entity = employeeCareDao
