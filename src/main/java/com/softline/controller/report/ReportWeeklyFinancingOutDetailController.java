...
@Controller
@RequestMapping("/reportWeeklyFinancingOutDetail")
/**
 * 
 * @author tch
 *
 */
public class ReportWeeklyFinancingOutDetailController {
	...
	public String _list(String stringList, Integer pageNums, String op, HttpServletRequest request ,Map<String, Object> map) throws IOException {
		...
		List<ReportWeeklyFinancingOutDetail> list = new ArrayList<ReportWeeklyFinancingOutDetail>();
		if(stringList != null && !"".equals(stringList))
			list=JSON.parseArray(stringList, ReportWeeklyFinancingOutDetail.class);
        //根据list，设置分页
		...
		List<ReportWeeklyFinancingOutDetail> newList = new ArrayList<ReportWeeklyFinancingOutDetail>();
	    ...
	    //控制分页
	    map.put("flag", "reportWeeklyFinancingOutDetail");
	    map.put("searchurl", "/shanghai-gzw/reportWeeklyFinancingOutDetail/list");
		addData(map);
		return "/report/reportCashflowWeeklyExecute/reportWeeklyFinancingOutDetail/reportWeeklyFinancingOutDetailList";
	}
	...
	public String _view(String stringList, Integer pageNums,HttpServletRequest request ,Map<String, Object> map,String op) throws IOException {
	    ...
	    //控制分页
	    map.put("flag", "reportWeeklyFinancingOutDetail");
	    map.put("searchurl", "/shanghai-gzw/reportWeeklyFinancingOutDetail/viewList");
		return "/report/reportCashflowWeeklyExecute/reportWeeklyFinancingOutDetail/reportWeeklyFinancingOutDetailViewList";
	}
	...
	public String exportExcel(String excelName, String excelData, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		List<ReportWeeklyFinancingOutDetail> dataList = new ArrayList<ReportWeeklyFinancingOutDetail>();
		if(excelData != null && !"".equals(excelData))
			dataList=JSON.parseArray(excelData, ReportWeeklyFinancingOutDetail.class);
	...
	private List<Map<String, Object>> createExcelRecord(List<ReportWeeklyFinancingOutDetail> dataList) {
    	...
    	for (int i = 0; i < 500; i++) {//默认给500条
    		 if(i<j) {
	        	ReportWeeklyFinancingOutDetail detail=dataList.get(i);
	...
	public void dealSelect(List<Object> listob) {
		...
		for(Object obj:listob) {
			ReportWeeklyFinancingOutDetail detail = (ReportWeeklyFinancingOutDetail)obj;
			for(HhBase base:financingType){
