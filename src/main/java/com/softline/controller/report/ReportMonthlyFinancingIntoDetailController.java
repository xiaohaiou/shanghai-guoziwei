...
@Controller
@RequestMapping("/reportMonthlyFinancingIntoDetail")
/**
 * 
 * @author tch
 *
 */
public class ReportMonthlyFinancingIntoDetailController {
	...
	public String _list(String stringList, Integer pageNums, String op, HttpServletRequest request ,Map<String, Object> map) throws IOException {
		...
		List<ReportMonthlyFinancingIntoDetail> list = new ArrayList<ReportMonthlyFinancingIntoDetail>();
		if(stringList != null && !"".equals(stringList))
			list=JSON.parseArray(stringList, ReportMonthlyFinancingIntoDetail.class);
        //根据list，设置分页
		...
		List<ReportMonthlyFinancingIntoDetail> newList = new ArrayList<ReportMonthlyFinancingIntoDetail>();
	    ...
	    //控制分页
	    map.put("flag", "reportMonthlyFinancingIntoDetail");
	    map.put("searchurl", "/shanghai-gzw/reportMonthlyFinancingIntoDetail/list");
		addData(map);
		return "/report/reportCashflowMonthlyExecute/reportMonthlyFinancingIntoDetail/reportMonthlyFinancingIntoDetailList";
	}
	...
	public String _view(String stringList, Integer pageNums,HttpServletRequest request ,Map<String, Object> map,String op) throws IOException {
	    ...
	    //控制分页
	    map.put("flag", "reportMonthlyFinancingIntoDetail");
	    map.put("searchurl", "/shanghai-gzw/reportMonthlyFinancingIntoDetail/viewList");
		return "/report/reportCashflowMonthlyExecute/reportMonthlyFinancingIntoDetail/reportMonthlyFinancingIntoDetailViewList";
	}
	...
	public String exportExcel(String excelName, String excelData, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		List<ReportMonthlyFinancingIntoDetail> dataList = new ArrayList<ReportMonthlyFinancingIntoDetail>();
		if(excelData != null && !"".equals(excelData))
			dataList=JSON.parseArray(excelData, ReportMonthlyFinancingIntoDetail.class);
	...
	//数据转换到map里面，调用公共方法
	private List<Map<String, Object>> createExcelRecord(List<ReportMonthlyFinancingIntoDetail> dataList) {
    	...
    	for (int i = 0; i < 500; i++) {//默认给500条
    		 if(i<j) {
	        	ReportMonthlyFinancingIntoDetail detail=dataList.get(i);
	...
	public void dealSelect(List<Object> listob) {
		...
		for(Object obj:listob) {
			ReportMonthlyFinancingIntoDetail detail = (ReportMonthlyFinancingIntoDetail)obj;
			for(HhBase base:sequelNew){
